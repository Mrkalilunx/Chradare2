...
static RCoreHelpMessage help_msg_d = {
	...
	"d:", "[?] [cmd]", "运行自定义调试插件命令",
	"db", "[?]", "断点命令",
	"dbt", "[?]", "基于 dbg.btdepth 和 dbg.btalgo 显示回溯",
	"dc", "[?]", "继续执行",
	"dd", "[?][*+-tsdfrw]", "管理子进程的文件描述符",
	"de", "[-sc] [perm] [rm] [e]", "使用 ESIL 调试 (参见 de?)",
	"dg", " <file>", "生成核心转储文件 (开发中)",
	"dh", " [plugin-name]", "选择新的调试处理器插件 (参见 dbh)",
	"dH", " [handler]", "将进程移植到新的处理器",
	"di", "[?]", "显示调试器后端信息 (参见 dh)",
	"dk", "[?]", "列出、发送、获取、设置子进程的信号处理器",
	"dL", "[?]", "列出或设置调试器处理器",
	"dm", "[?]", "显示内存映射",
	"do", "[?]", "打开进程 (重新加载，'oo' 的别名)",
	"doo", "[args]", "使用参数重新以调试模式打开 ('ood' 的别名)",
	"doof", "[file]", "从文件重新以调试模式打开 ('oodf' 的别名)",
	"doc", "", "关闭调试会话",
	"dp", "[?]", "列出、附加到进程或线程ID",
	"dr", "[?]", "CPU 寄存器",
	"ds", "[?]", "单步步入、步过、源代码行",
...
static RCoreHelpMessage help_msg_dbi = {
	"用法: dbi", "", " # 通过索引操作断点",
	"dbi", "", "列出断点索引",
	"dbi", " <addr>", "显示给定偏移处的断点索引",
	"dbi.", "", "显示当前偏移处的断点索引",
	"dbi-", " <idx>", "按索引删除断点",
	"dbix", " <idx> [expr]", "为给定索引的断点设置表达式",
	"dbic", " <idx> <cmd>", "在断点索引处运行命令",
	"dbie", " <idx>", "按索引启用断点",
	"dbid", " <idx>", "按索引禁用断点",
	"dbis", " <idx>", "交换第 N 个断点",
	"dbite", " <idx>", "按索引启用断点跟踪",
	"dbitd", " <idx>", "按索引禁用断点跟踪",
	"dbits", " <idx>", "交换第 N 个断点跟踪",
	NULL
};
...
static RCoreHelpMessage help_msg_db = {
	"用法: db", "", " # 断点命令",
	"db", "", "列出断点",
	"db", " addr", "添加断点 (参见 dbg.hwbp)",
	"db-", "*|addr", "删除地址处的断点 (或所有断点*)",
	"db.", "", "显示当前地址的断点详情",
	"db*", "", "以 radare 命令列出断点",
	"dbj", "", "以 JSON 格式列出断点",
	// "dbi", " 0x848 ecx=3", "条件匹配时停止执行",
	"db", "[c|C] addr cmd", "命中断点时运行命令 (dbC, 命中后继续)",
	"db", "[d|e] addr", "通过地址禁用或启用断点",
	"dbf", "", "在每个无返回函数中设置断点", // XXX 名称不佳
	"dbh", "[-| x86]", "使用/移除/列出断点插件处理器",
	"dbH", " [addr]", "添加硬件断点",
	"dbi", "[?]", "通过索引操作断点",
	"dbm", " <module> <offset>", "在模块基址偏移处添加断点",
	"dbn", " [<name>]", "显示或设置当前断点的名称",
	"dbs", " <addr>", "切换断点",
	"dbt", "[?]", "显示回溯。参见 dbt? 获取更多详情",
	"dbx", " [expr]", "为当前偏移处的断点设置表达式",
	"dbw", " <addr> <r/w/rw>", "添加观察点",
#if __WINDOWS__
	"dbW", " <WM_DEFINE> [?|handle|name]", "在窗口消息处理器上设置条件断点",
#endif
	"dbx", " number addr len perm", "修改硬件断点",
	"dbx-", "number", "清除硬件断点",
	NULL
};
...
static RCoreHelpMessage help_msg_dbt = {
	...
	"dbt", "", "基于 dbg.btdepth 和 dbg.btalgo 显示回溯",
	"dbt*", "", "以标志形式显示回溯",
	"dbt=", "", "单行显示回溯 (参见 dbt=s 和 dbt=b 分别用于 sp 或 bp)",
	"dbtv", "", "显示回溯及局部变量（如果有）",
	"dbtj", "", "以 JSON 格式显示回溯",
	"dbta", "", "以 ASCII 艺术形式显示栈回溯",
	"dbte", " <addr>", "启用断点跟踪",
	"dbtd", " <addr>", "禁用断点跟踪",
	"dbts", " <addr>", "切换断点跟踪",
	NULL
...
static RCoreHelpMessage help_msg_drr = {
	"用法: drr", "", " # 显示寄存器引用 (参见 pxr?)",
	"drr", "", "显示寄存器值的关联引用",
	"drrj", "", "同上，但以 JSON 格式输出",
	NULL
};

static RCoreHelpMessage help_msg_dbw = {
	"用法: dbw", "<addr> <r/w/rw>", " # 添加观察点",
	NULL
};
...
static RCoreHelpMessage help_msg_dc = {
	...
	"dce", "", "继续执行 (将异常传递给程序)",
	"dcf", "", "继续直到 fork (待实现)",
	"dck", " <signal> <pid>", "继续向进程发送信号",
	"dcp", "", "继续直到程序代码 (映射的 IO 区段)",
	"dcr", "", "继续直到返回 (使用步过)",
	"dcs", "[?] <num>", "继续直到系统调用",
	"dct", " <len>", "从当前搜索位置到指定长度的陷阱跟踪，无参数则列出",
	"dcu", "[?] [..end|addr] ([end])", "继续直到地址 (或范围)",
	/*"TODO: dcu/dcr 需要 dbg.untilover=true??",*/
	/*"TODO: 同样适用于仅用户/库侧，避免步入库",*/
	/*"TODO: 支持线程?",*/
	NULL
};
...
static RCoreHelpMessage help_msg_dcs = {
	"用法:", "dcs", " 继续直到系统调用",
	"dcs", "", "继续直到下一个系统调用",
	"dcs [str]", "", "继续直到下一个 'str' 系统调用",
	"dcs", "*", "跟踪所有系统调用，类似 strace",
	NULL
};

static RCoreHelpMessage help_msg_dcu = {
	"用法:", "dcu", " 继续直到地址",
	"dcu.", "", "dcu $$ 的别名 (继续直到当前地址)",
	"dcu", " address", "继续直到地址",
	"dcu", " [..tail]", "继续直到范围",
	"dcu", " [from] [to]", "继续直到范围",
	NULL
...
static RCoreHelpMessage help_msg_dd = {
	"用法: dd", "", "管理子进程的文件描述符 (* 显示 r2 命令)",
	"dd", "[*]", "列出文件描述符",
	"dd", "[*] <file|addr>", "以只读方式打开文件 (r--)；addr = 用作路径的 char*",
	"dd+", "[*] <file|addr>", "以读写方式打开/创建文件 (rw-)；addr = 用作路径的 char*",
	"dd-", "[*] <fd>", "关闭 fd",
	"ddt", "[*]", "关闭终端 fd (`dd- 0` 的别名)",
	"dds", "[*] <fd> [offset]", "将 fd 定位到偏移 (无偏移 = 定位到开头)",
	"ddd", "[*] <oldfd> <newfd>", "使用 dup2 将 oldfd 复制到 newfd",
	"ddf", "[*] <addr>", "创建管道并将 fd 写入 (int[2])addr",
	"ddr", "[*] <fd> <addr> <size>", "从 fd 读取字节到 (char*)addr",
	"ddw", "[*] <fd> <addr> <size>", "从 (const char*)addr 写入字节到 fd",
	NULL
};

static RCoreHelpMessage help_msg_de = {
	"用法:", "de", "[-sc] [perm] [rm] [expr]",
	"de", "", "列出 ESIL 观察点",
	"de-*", "", "删除所有 ESIL 观察点",
	"de ", "[perm] [rm] [addr|reg|from..to]", "条件满足时停止",
	"dec", "", "继续执行直到匹配表达式",
	"des", "[?] [N]", "使用 ESIL 调试步入 N 条指令",
	"desu", " [addr]", "ESIL 调试直到指定地址",
	NULL
};
...
static RCoreHelpMessage help_msg_dk = {
	"用法: dk", "", "信号命令",
	"dk", "", "列出子进程的所有信号处理器",
	"dk", " <signal>", "向子进程发送 KILL 信号",
	"dk", " <signal>=1", "为子进程中的 <signal> 设置信号处理器",
	"dk?", "<signal>", "信号名/信号编号解析器",
	"dko", "[?] <signal>", "重置给定信号的跳过或继续选项",
	"dko", " <signal> [|skip|cont]", "遇到信号时跳过处理器或继续执行",
	"dkj", "", "以 JSON 格式列出所有信号处理器",
	NULL
};
...
static RCoreHelpMessage help_msg_dm = {
	"用法:", "dm", " # 内存映射命令",
	"dm", "", "列出目标进程的内存映射",
	"dm", " address size", "在子进程中的 <address> 处分配 <size> 字节 (如果地址为 -1 则在任意位置)",
	"dm=", "", "列出目标进程的内存映射 (ASCII 艺术条)",
	"dm.", "", "显示当前地址的映射名称",
	"dm*", "[-*]", "以 radare 命令列出内存映射 (dm** 每个调试映射的 ioMap，dm*- 撤销)",
	"dm-", " address", "释放 <address> 的内存映射",
	"dmd", "[aw] [file]", "将当前（全部或可写）调试映射区域转储到文件 (from-to.dmp) (参见 Sd)",
	"dmh", "[?]", "显示堆映射",
	"dmi", "[?] ([addr|libname] [symname])", "列出或从库加载符号",
	"dmj", "", "以 JSON 格式列出内存映射",
	"dml", " <file>", "将文件内容加载到当前映射区域",
	"dmm", "[?][j*]", "列出模块 (内存中加载的库、二进制文件)",
	"dmp", "[?] <address> <size> <perms>", "使用 <size> 和权限 <perms> 更改 <address> 处的页面",
	"dms", "[?] <id> <mapaddr>", "获取/存储内存快照",
	"dms-", " <id> <mapaddr>", "恢复内存快照",
	"dmS", "[*] [addr|libname] [sectname]", "列出目标库的区段",
	"dmL", " address size", "在 <address> 处分配 <size> 字节并提升为大页面",
	//"dm, " rw- esp 9K", "将 9KB 的栈设置为读+写 (无执行)",
	// "TODO:", "", "在进程内存中映射文件。 (dmf file @ [addr])",
	NULL
};
...
static RCoreHelpMessage help_msg_dmi = {
	"用法: dmi", "", " # 列出/加载符号",
	"dmi.", "", "列出最接近当前地址的符号",
	"dmi", "[j|q|*] [libname] [symname]", "列出目标库的符号",
	"dmi*", "", "以 radare 命令列出目标库的符号",
	"dmia", "[j|q|*] [libname]", "列出目标库的所有信息",
	"dmis", " [libname]", "与 .dmi* 相同 - 从给定库导入所有符号作为标志",
	"dmiv", "", "显示给定库中给定符号的地址",
	NULL
};
...
static RCoreHelpMessage help_msg_dmm = {
	"用法:", "dmm", " # 模块内存映射命令",
	"dmm", "", "列出目标进程的模块",
	"dmm*", "", "以 radare 命令列出目标进程的模块",
	"dmm.", "", "列出当前模块的内存映射",
	"dmmj", "", "以 JSON 格式列出目标进程的模块",
	NULL
};
...
static RCoreHelpMessage help_msg_dms = {
	...
	"dms*", "", "以 r2 命令列出快照",
	"dmsj", "", "以 JSON 格式列出快照",
	"dmsC", " id comment", "为给定快照添加注释",
	"dmsd", " id", "十六进制差异比较给定快照。参见 `ccc`。",
	"dmsw", "", "可写映射的快照",
	"dmsa", "", "所有 `dm` 映射的完整快照",
	NULL
};
...
static RCoreHelpMessage help_msg_dxe = {
	"用法:", "dxe", " egg-program # 参见 ragg2 和 'g' 命令获取更多详情",
	"dxe", " sym.imp.puts(\"foo\")", "使用字符串参数调用 puts",
	NULL
};

static RCoreHelpMessage help_msg_dmp = {
	"用法:", "dmp", " 更改页面权限",
	"dmp", " [addr] [size] [perms]", "更改权限",
	"dmp", " [perms]", "更改 dbg.map 权限",
	"dmp.", "", "显示当前映射的权限",
	NULL
};
...
static RCoreHelpMessage help_msg_do = {
	"用法:", "do", " # 调试（重新）打开命令",
	"do", "", "打开进程 (重新加载，'oo' 的别名)",
	"dor", " [rarun2]", "以逗号分隔的 k=v rarun2 配置文件选项列表 (e dbg.profile)",
	"doe", "", "显示 rarun2 启动配置文件",
	"doe!", "", "使用 $EDITOR 编辑 rarun2 启动配置文件",
	"doo", " [args]", "使用参数以调试模式重新打开 ('ood' 的别名)",
	"doof", " [args]", "从文件以调试模式重新打开 ('oodf' 的别名)",
	"doc", "", "关闭调试会话",
	NULL
};
...
static RCoreHelpMessage help_msg_dp = {
	"用法:", "dp", " # 进程命令",
	"dp", "", "列出当前 pid 和子进程",
	"dp", "q", "与 dp 相同。仅显示当前进程 id",
	"dp", " <pid>", "列出 pid 的子进程",
	"dpj", " <pid>", "以 JSON 格式列出 pid 的子进程",
	"dpl", "", "列出所有可附加的 pid",
	"dplj", "", "以 JSON 格式列出所有可附加的 pid",
	"dp-", " <pid>", "分离选定的 pid",
	"dp=", "<pid>", "选择 pid",
	"dpa", " <pid>", "附加并选择 pid",
	"dpc", "", "选择分叉的 pid (参见 dbg.forks)",
	"dpc*", "", "显示分叉的 pid (参见 dbg.forks)",
	"dpe", "", "显示可执行文件路径",
	"dpf", "", "像文件 fd 一样附加到 pid // 黑客方法",
	"dpk", " <pid> [<signal>]", "向进程发送信号 (默认 0)",
	"dpn", "", "创建新进程 (fork)",
	"dpt", "[?][j]", "列出当前 pid 的线程",
	"dpt", " <pid>", "列出进程的线程",
	"dpt.", "", "显示当前线程 id",
	"dptj", "", "以 JSON 格式列出当前 pid 的线程",
	"dptn", "", "创建新线程 (clone)",
	"dptj", " <pid>", "以 JSON 格式列出进程的线程",
	"dpt=", "<thread>", "附加到线程",
	NULL
};
...
static RCoreHelpMessage help_msg_dr = {
	"用法: dr", "", "寄存器命令",
	"dr", "", "显示 'gpr' 寄存器",
	"dr", " <register>=<val>", "设置寄存器值",
	"dr.", " >$snapshot", "在当前寄存器值中捕获 r2 别名文件",
	"dr,", " [table-query]", "以表格格式枚举寄存器",
	"dr0", "", "零填充寄存器区域，重置其状态",
	"dr8", "[1|2|4|8] [type]", "显示 gpr 区域的十六进制转储 (开发中)",
	"dr=", "", "以列形式显示寄存器",
	"dr?", "<register>", "显示给定寄存器的值",
	"dr??", "", "与 dr?`drp~=[0]+` 相同 # 列出所有寄存器角色别名名称和值",
	"dra", "[?]", "管理寄存器区域。参见 ara?",
	"drb", "[1|2|4|8] [type]", "显示 gpr 区域的十六进制转储 (开发中)",
	"drc", " [name]", "与条件标志寄存器相关",
	"drC", " [register]", "显示寄存器注释",
	"drd", "", "仅显示不同的寄存器",
	"dre", "", "显示设置寄存器值的 esil 表达式 (类似 dr*)",
	"drf", "", "显示 FPU 寄存器 (80 位长双精度)",
	"dri", "", "显示反向寄存器转储 (按值排序)",
	"drl", "[j]", "列出所有寄存器名称",
	"drv", "[?]", "显示向量寄存器 (也称为 sve / 打包 / 向量)",
	"dro", "", "显示寄存器的先前（旧）值",
	"drn", "", "列出、显示或更改寄存器别名名称 (PC,A0, 由寄存器配置文件定义)",
	"drp", "[?] ", "显示当前寄存器配置文件",
	"drr", "", "显示寄存器引用 (望远镜式)",
	"drrj", "", "以 JSON 格式显示寄存器引用 (望远镜式)",
	// TODO: 'drs' 交换寄存器区域并显示旧寄存器值
	"drs", "[?]", "栈寄存器状态",
	"drt", "[?]", "显示所有寄存器类型",
	"drw", " <hexnum>", "设置寄存器区域的内容",
	"drx", "[?]", "显示调试寄存器",
	".dr", "*", "在标志中包含常用寄存器值",
	".dr", "-", "取消标记所有寄存器",
	NULL
...
static RCoreHelpMessage help_msg_drp = {
	"用法:", "drp", " # 寄存器配置文件命令",
	"drp", "", "显示当前寄存器配置文件",
	"drp", " [regprofile-file]", "设置当前寄存器配置文件",
	"drp", " [gdb] [regprofile-file]", "解析 gdb 寄存器配置文件并转储 r2 配置文件字符串",
	"drp*", "", "打印为区域偏移内的每个寄存器创建标志的 r2 命令",
	"drpc", "", "显示寄存器配置文件注释",
	"drpi", "", "显示寄存器配置文件的内部表示",
	"drp.", "", "显示当前伪造大小",
	"drpj", "", "显示当前寄存器配置文件 (JSON)",
	"drps", " [new fake size]", "获取或设置寄存器配置文件大小",
	"drpg", "", "显示寄存器配置文件注释",
	"注意:", "", "当 cfg.debug=0 时运行 drp? 将显示 arp",
	NULL
};
...
static RCoreHelpMessage help_msg_drt = {
	"用法:", "drt", " [type] [size]    # 调试寄存器类型",
	"drt", "", "列出所有可用的寄存器类型",
	"drt", " [size]", "显示配置文件中指定大小的所有寄存器",
	"drt", " 16", "显示 16 位寄存器",
	"drt", " [type]", "显示配置文件中此类型的所有寄存器",
	"drt", " all", "显示所有寄存器",
	"drt", " fpu", "显示 FPU 寄存器",
	"drt", " [type] [size]", "同上，针对类型和大小",
	"drt", " [type] [size]", "同上，针对类型和大小",
	"drt*", "", "以 r 命令列出标志",
	NULL
};
...
static RCoreHelpMessage help_msg_drx = {
	"用法: drx", "", "硬件断点命令",
	"drx", "", "列出所有 (x86?) 硬件断点",
	"drx", " <number> <address> <length> <perms>", "修改硬件断点",
	"drx-", "<number>", "清除硬件断点",
	NULL
};

static RCoreHelpMessage help_msg_drv = {
	"用法: drv", " [reg] [idx] [wordsize] [= value]", "显示向量打包寄存器",
	"drv", "", "显示 XMM 寄存器",
	"drv", " xmm0", "显示 xmm0 的所有打包方式",
	"drv", " xmm0 0 32 = 12", "将 xmm0 寄存器的第一个 32 位字设置为 12",
	"drvb", " [reg]", "以字节形式显示寄存器",
	"drvw", " [reg]", "以字形式显示寄存器",
	"drvd", " [reg]", "以双字形式显示寄存器",
	"drvq", " [reg]", "以四字形式显示寄存器",
	"drvq", " xmm0~[0]", "显示 xmm0 的第一个四字",
	"drvf", " [reg]", "以 32 位浮点数形式显示寄存器",
	"drvl", " [reg]", "以 64 位浮点数形式显示寄存器",
	"drvyb", " [reg]", "以字节形式显示 YMM 寄存器",
	"drvyw", " [reg]", "以字形式显示 YMM 寄存器",
	"drvyd", " [reg]", "以双字形式显示 YMM 寄存器",
	"drvyq", " [reg]", "以四字形式显示 YMM 寄存器",
	"drvq", " ymm0~[3]", "显示 ymm0 的第四个四字",
	"drvyf", " [reg]", "以 32 位浮点数形式显示 YMM 寄存器",
	"drvyl", " [reg]", "以 64 位浮点数形式显示 YMM 寄存器",
	NULL
};
...
static RCoreHelpMessage help_msg_ds = {
	...
	"dsi", " <cond>", "继续直到条件匹配",
	"dsl", "", "单步执行一行源代码",
	"dsl", " <num>", "单步执行 <num> 行源代码",
	"dso", " <num>", "步过 <num> 条指令",
	"dsp", "", "步入程序 (跳过库)",
	"dss", " <num>", "跳过 <num> 条单步指令",
	"dsu", "[?] <address>", "单步直到 <address>。参见 'dsu?' 获取其他单步直到命令。",
	NULL
};

static RCoreHelpMessage help_msg_dsu = {
	"用法: dsu", "", "单步直到命令",
	"dsu ", "<address>", "单步直到 <address>",
	"dsui", " <instr>", "单步直到指令反汇编匹配",
	"dsuir", " <regex>", "类似 dsui，但使用正则表达式",
	"dsuo", " <optype> [<optype> ...]", "单步直到指令匹配其中一个 <optype>",
	"dsue", " <esil>", "单步直到 <esil> 表达式匹配",
	"dsuf", " <flag>", "单步直到 pc == 匹配名称的 <flag>",
	NULL
};
...
static RCoreHelpMessage help_msg_dt = {
	...
	"dt", " [addr]", "显示地址处的跟踪信息",
	"dt*", "", "列出所有跟踪的操作码偏移",
	"dtj", "", "以 json 格式列出指令跟踪日志",
	"dt+", " [addr] [times]", "为地址添加跟踪 N 次",
	"dt-", "", "重置跟踪 (指令/调用)",
	"dt=", "", "显示带有调试跟踪范围的 ASCII 艺术颜色条",
	"dta", " 0x804020 ...", "仅跟踪给定地址",
	"dtc", "[?][addr]|([from] [to] [addr])", "跟踪调用/返回",
	"dtd", "[qi] [nth-start]", "列出所有跟踪的反汇编 (安静, 指令)",
	"dte", "[?]", "显示 ESIL 跟踪日志",
	"dtg", "", "图形化调用/返回跟踪",
	"dtg*", "", "以 agn/age 命令图形化。使用 .dtg*;aggi 进行可视化",
	"dtgi", "", "交互式调试跟踪",
	"dts", "[?]", "管理跟踪会话，用于步回 (实验性)",
	"dtt", " [tag]", "选择跟踪标签 (无参数则取消设置)",
	"dtt.", "", "显示当前标签",
	NULL
};
...
static RCoreHelpMessage help_msg_dte = {
	...
	"dtei", "", "模拟指令并记录其效果的跟踪",
	"dted", "", "反汇编所有跟踪的指令",
	"dtek", " [sdb query]", "从 sdb 获取单条指令的 ESIL 跟踪日志",
	NULL
};
...
static RCoreHelpMessage help_msg_dx = {
	...
	"dx", " <hexpairs>", "执行操作码",
	"dxa", " <asm>", "汇编代码并执行",
	"dxe", "[?] <egg-expr>", "编译 egg 表达式并执行",
	"dxr", " <hexpairs>", "执行操作码并恢复状态",
	"dxrs", " <hexpairs>", "执行操作码并恢复状态，不包括栈",
	"dxs", " <name> [args]", "系统调用注入 (参见 gs)",
	"\n示例:", "", "",
	"dx", " 9090", "执行两个 x86 nop",
	"\"dxa mov eax,6;mov ebx,0;\"", "", "汇编并执行",
	"dxs", " write 1, 0x8048, 12", "从 0x8048 向 stdout 写入 12 字节",
	NULL
};
