...
static RCoreHelpMessage help_msg_w = {
	"用法:", "w[x] [str] [<file] [<<EOF] [@addr]", "",
	"w ", "foobar", "写入字符串 'foobar'",
	"w+", "string", "写入字符串并定位到其空终止符",
	"w0", " [len]", "写入 'len' 个值为 0x00 的字节",
	"w6", "[d|e|x] base64/string/hex", "写入 base64 [d]解码或[e]编码的字符串",
	"w8", " [hexpairs]", "wx 的别名",
	"wa", "[?] push ebp", "写入操作码，用 ';' 分隔（在命令周围使用 '\"'）",
	"waf", " f.asm", "汇编文件并写入字节",
	"waF", " f.asm", "汇编文件并写入字节，并显示汇编代码十六进制对的 'wx' 操作",
	"wao", "[?] op", "修改操作码（更改跳转条件、nop 等）（RArch.patch）",
	//"wA", "[?] r 0", "在当前搜索处修改操作码（参见 wA?）",
	"wb", " 011001", "以大端位写入位（参见 pb）",
	"wB", "[-]0xVALUE", "设置或取消设置具有给定值的位（也支持 wB-0x2000）",
	"wc", "[?][jir+-*?]", "写入缓存列表/撤销/提交/重置（io.cache）",
	"wd", " [off] [n]", "从 OFF 复制 N 字节到 $$（memcpy）（参见 y?）",
	"we", "[?] [nNsxX] [arg]", "扩展写入操作（插入而不是替换）",
	"wf", "[fs] -|file", "在当前偏移处写入文件内容",
	"wg", "[et] [http://host/file]", "从 http 服务器下载文件并保存到磁盘（wget）",
	"wh", " r2", "whereis/which shell 命令",
	"wi", "[1248][+-][n]", "递增/递减字节、字等",
	"wm", " f0ff", "设置二进制掩码十六进制对用作循环写入掩码",
	"wo", "[?] hex", "在块中执行操作写入。'wo?' fmi",
	"wp", "[?] -|file", "应用 radare 补丁文件。参见 wp? fmi",
	"wr", " 10", "写入 10 个随机字节",
	"ws", "[?] pstring", "写入 Pascal 字符串：1 字节长度 + N 字节字符串",
	"wt", "[?][afs] [filename] [size]", "写入文件（从当前搜索、块大小或 sz 字节开始）",
	"ww", " foobar", "写入宽字符串 'f\\x00o\\x00o\\x00b\\x00a\\x00r\\x00'",
	"wx", "[?][fs] 9090", "写入两个 intel nop（来自 wxfile 或 wxseek）",
	"wX", " 1b2c3d", "用循环十六进制对填充当前块",
	"wv", "[?] [expr]", "使用 cfg.bigendian 写入 [1,2,4,8] 字节大小",
	"wu", " [unified-diff-patch]", "参见 'cu'",
	"wz", " string", "写入零终止字符串（类似 w + \\x00）",
	NULL
};

static RCoreHelpMessage help_msg_wao = {
	"wao", " [op]", "对当前操作码执行修改",
	"waol", " [op]", "补丁的字节长度（例如 intel nop 是 1 字节）",
	"wao*", " [op]", "显示将执行的命令以应用补丁",
	"wao+", " [op]", "同 'wao'，但在写入后向前定位",
	"wao", " nop", "nop 当前操作码",
	"wao", " jinf", "汇编无限循环",
	"wao", " jz", "使当前操作码有条件（同 je）（零）",
	"wao", " jnz", "使当前操作码有条件（同 jne）（非零）",
	"wao", " ret1", "使当前操作码返回 1",
	"wao", " ret0", "使当前操作码返回 0",
	"wao", " retn", "使当前操作码返回 -1",
	"wao", " nocj", "从分支移除条件操作（使其无条件）",
	"wao", " trap", "使当前操作码成为陷阱",
	"wao", " recj", "反转（交换）条件分支指令",
	"进行中:", "", "并非所有架构都支持，且并非所有命令在所有架构上都有效",
	NULL
};

static RCoreHelpMessage help_msg_ws = {
	"用法:", "ws[124?] [string]", "Pascal 字符串不以空字符终止，而是在开头以二进制存储长度",
	"ws", " str", "使用第一个字节作为长度写入 Pascal 字符串",
	"ws1", " str", "同上",
	"ws2", " str", "同上但使用 ut16 作为长度（遵循 cfg.bigendian）",
	"ws4", " str", "同上，但使用 ut32（遵循 cfg.bigendian）",
	NULL
};

static RCoreHelpMessage help_msg_wa = {
	"用法:", "wa[of*] [arg]", "",
	"wa", " nop", "使用 asm.arch 和 asm.bits 写入 nop 操作码",
	"wai", " jmp 0x8080", "在此操作内写入（用 nop 填充，如果不合适则报错）",
	"wan", " jmp 0x8080", "写入指令并用 nop 填充尾部字节",
	"wa+", " nop", "写入 nop 并在其后定位（使用 7wa+nop 写入 7 个连续 nop）",
	"wa*", " mov eax, 33", "显示汇编操作码十六进制对的 'wx' 操作",
	"'wa nop;nop", "" , "汇编多个指令（注意单引号）",
	"waf", " f.asm" , "汇编文件并写入字节",
	"waF", " f.asm", "汇编文件并写入字节，并显示汇编代码十六进制对的 'wx' 操作",
	"waF*", " f.asm", "汇编文件并显示汇编代码十六进制对的 'wx' 操作",
	"wao?", "", "显示当前操作码汇编器操作的帮助（hack）",
	NULL
};

static RCoreHelpMessage help_msg_wc = {
	"用法:", "wc[jir+-*?]", "  # 参见 `e io.cache = true`",
	"wc", "", "列出当前缓存层中的所有写入更改",
	"wc*", "", "打印复制当前缓存层中补丁的写入命令",
	"wc**", "", "同 'wc*' 但针对所有缓存层",
	"wc+", " [from] [to]", "将更改从缓存提交到 io",
	"wc++", "", "推送新的 io 缓存层",
	"wc-", " [from] [to]", "移除当前搜索或给定地址的写入操作",
	"wc--", "", "弹出（丢弃）最后一个写入缓存层",
	"wcU", "", "重做已撤销的更改（TODO）",
	"wca", "", "列出所有缓存层中的所有写入更改",
	"wcd", "", "以反汇编差异格式列出所有写入更改",
	"wcf", " [file]", "将写入缓存提交到给定文件",
	"wci", "", "提交写入缓存",
	"wcj", "", "以 JSON 格式列出所有写入更改",
	"wcl", "", "列出 io 缓存层",
	"wcp", " [fd]", "列出指定 fd 或当前 fd 上 p 层的所有缓存写入操作",
	"wcp*", " [fd]", "以 radare 命令列出 p 层中的所有缓存写入操作",
	"wcpi", " [fd]", "提交并使指定 fd 或当前 fd 的 pcache 无效",
	"wcr", "", "恢复缓存中的所有写入",
	"wcs", "", "压缩连续的写入操作",
	"wcu", "", "撤销最后一次更改",
	NULL
...
static RCoreHelpMessage help_msg_we = {
	"用法", "", "写入扩展 # 调整文件大小",
	"wen", " <num>", "扩展底层文件，在当前偏移处插入 NUM 个空字节",
	"weN", " <addr> <len>", "扩展当前文件并在地址处插入字节",
	"wes", " <addr>  <dist> <block_size>", "左移块大小或在编辑器中写入",
	"wex", " <hex_bytes>", "通过扩展文件在当前偏移处插入字节",
	"weX", " <addr> <hex_bytes>", "通过扩展文件在地址处插入字节",
	NULL
};

static RCoreHelpMessage help_msg_wo = {
	"用法:", "wo[asmdxoArl24]", " [hexpairs] @ addr[!bsize] 在当前块中执行写入操作",
	"wo2", "", "2= 2 字节字节序交换（字）",
	"wo4", "", "4= 4 字节字节序交换（双字）",
	"wo8", "", "8= 8 字节字节序交换（四字）",
	"woa", " [hexpair]", "+= 加法（例如：woa 0102）",
	"woA", " [hexpair]", "&= 与",
	"wod", " [hexpair]", "/= 除法",
	"woD", " [algo] [key] [IV]", "使用给定算法和密钥解密当前块",
	"woE", " [algo] [key] [IV]", "使用给定算法和密钥加密当前块",
	"woe", " [from] ([to] [step] [wsz=1])", "写入枚举序列 i0 01 02 ..",
	"woi", "", "反转当前块中的字节",
	"wol", " [val]", "<<= 左移",
	"wom", " [val]", "*= 乘法",
	"woo", " [val]", "|= 或",
	...
	"woR", "", "随机字节（'wr $b' 的别名）",
	"wos", " [val]", "-= 减法",
	"woS", " [algo] [key]", "使用给定算法和密钥签名当前块",
	"wow", " [val]", "== 写入循环值（'wb' 的别名）",
	"wox", " [val]", "^= 异或（例如：wox 0x90）",
	NULL
};

static RCoreHelpMessage help_msg_wop = {
	"用法:", "wop[DO]", " len @ addr | value",
	"wopD", " len [@ addr]", "在地址 'addr' 处写入长度为 'len' 的德布鲁因模式",
	"wopD*", " len [@ addr]", "显示创建特定长度德布鲁因模式的 wx 命令",
	"wopO", " value", "在当前偏移处的德布鲁因模式中查找给定值",
	NULL
};
...
static RCoreHelpMessage help_msg_wt = {
	"用法:", "wt[afs] [filename] [size]", " 将当前块或从偏移开始的 [size] 字节写入文件",
	"wta", " [filename]", "追加到 'filename'",
	"wtf", " [filename] [size]", "写入文件（另见 'wxf' 和 'wf?'）",
	"wtf!", " [filename]", "从当前地址写入文件到 eof（忽略给定大小）",
	"wtff", " [prefix] [size]", "从当前搜索将块写入 \"<prefix>-<offset>\"",
	"wts", " host:port [size]", "发送数据到远程套接字 tcp://host:port",
	"注意:", "", "文件名默认为 \"<cfg.prefixdump>.<offset>\"",
	NULL
};

static RCoreHelpMessage help_msg_wf = {
	"用法:", "wf[fs] [-|args ..]", " 从（文件、交换、偏移）写入",
	"wf", " 10 20", "从偏移 10 写入 20 字节到当前搜索",
	"wff", " file [len]", "将文件内容写入当前偏移",
	"wfs", " host:port [len]", "从套接字写入（在端口监听 tcp 获取 N 字节）",
	"wfx", " 10 20", "在当前偏移和 10 之间交换 20 字节",
	NULL
};

static RCoreHelpMessage help_msg_wv = {
	"用法:", "wv[size] [value]", " 写入给定大小的值",
	"wv", " 0x834002", "写入具有此值的双字",
	"wv1", " 234", "写入具有此值的一个字节",
	"wv2", " 234", "写入具有此数字的无符号短整型（2 字节）",
	"wv4", " 1 2 3", "写入 N 个空格分隔的双字（4 字节）",
	"wv8", " 234", "写入具有此数字的四字（8 字节）",
	"wvF", " 3.14", "写入双精度值（8 字节）",
	"wvf", " 3.14", "写入浮点值（4 字节）",
	"wvG", " 3.14", "写入长双精度值（10/16 字节）",
	"wvp", " 934", "写入 4 或 8 字节指针，取决于 asm.bits",
	"支持的尺寸:", "1, 2, 4, 8", "",
	NULL
};
...
static RCoreHelpMessage help_msg_wx = {
	...
	"wx", " 3.", "写入当前字节的左半字节",
	"wx", " .5", "写入当前字节的右半字节",
	"wx+", " 9090", "写入十六进制对并向前定位",
	"wxf", " -|file", "在此处写入十六进制对文件的内容",
	NULL
};

static void cmd_write_fail(RCore *core) {
	R_LOG_ERROR ("无法写入。请使用 `omp`、`io.cache` 或重新以 `oo+` 模式打开文件");
