...
static RCoreHelpMessage help_msg_t = {
	"用法: t", "", "解析、管理和打印 C 类型",
	"t", "", "列出所有加载的类型",
	"tj", "", "以 json 格式列出所有加载的类型",
	"t", " <type>", "以 'pf' 语法显示类型",
	"t*", "", "在 r2 命令中列出类型信息",
	"t-", " <name>", "按名称删除类型",
	"t-*", "", "移除所有类型",
	"tail", "([n]) [file]", "输出文件的最后 n 行 (默认 n=5)",
	"tac", " [file]", "臭名昭著的反向 cat 命令",
	"tc", "[?] [type.name]", "以 C 输出格式列出所有/给定类型",
	"td", " <string>", "从字符串加载类型 (引用整个命令: \"td ...\")",
	"te", "[?]", "列出所有加载的枚举",
	"tf", "[?]", "列出所有加载的函数签名",
	"tk", " <sdb-query>", "执行 sdb 查询",
	"tl", "[?]", "显示/链接类型到地址",
	"tn", "[?] [-][addr]", "管理无返回函数属性和标记",
	"to", "[?] <path>", "从 C 头文件加载类型",
	"tp", "[?] <type> [addr|varname]", "将 <address> 的数据转换为 <type> 并打印 (XXX: 类型可以包含空格)",
	"ts", "[?]", "打印加载的结构类型",
	"tt", "[?]", "列出所有加载的 typedefs",
	"tu", "[?]", "打印加载的联合类型",
	"tx", "[?]", "类型交叉引用",
...
static RCoreHelpMessage help_msg_tx = {
	...
	"tx", "", "列出函数及其使用的类型",
	"tx.", "", "与 txf 相同",
	"tx", " int32_t", "列出使用此类型的函数名称",
	"txt", " int32_t", "与 'tx type' 相同",
	"txf", " ([addr])", "列出当前或给定函数中使用的所有类型 (与 tx. 相同)",
	"txl", "", "列出任何函数使用的所有类型",
	"txg", "", "渲染类型交叉引用图 (用法 .txg;aggv)",
	NULL
};

static RCoreHelpMessage help_msg_tcc = {
	"用法: tcc", "[-name]", "类型函数调用约定 (另见 afc? 和 arcc)",
	"tcc", "", "列出所有调用约定",
	"tcc*", "", "以 r2 命令形式列出调用约定",
	"tcck", "", "以 k=v 格式列出调用约定",
	"tccl", "", "列出 cc 签名 (返回 ccname (arg0, arg1, ..) err;)",
	"tccj", "", "以 JSON 格式列出它们",
	"tcc ", "<ret> ([args]) ([err])", "定义函数 cc",
	"tcc ", "r0 pascal(r0,r1,r2)", "为 pascal cc 定义签名 (另见 arcc)",
	"tcc-", "<name>", "按名称注销调用约定",
	"tcc-*", "", "注销所有调用约定",
	NULL
};
...
static RCoreHelpMessage help_msg_tf = {
	...
	"tf", "", "列出所有加载的函数定义",
	"tf", " <name>", "显示函数签名",
	"tfc", " [name]", "以 C 输出格式列出所有/给定函数签名并换行",
	"tfcj", " <name>", "与上面相同，但以 JSON 格式",
	"tfj", "", "以 JSON 格式列出所有函数定义",
	"tfj", " <name>", "以 JSON 格式显示函数签名",
	NULL
};

static RCoreHelpMessage help_msg_to = {
	"用法: to[...]", "", "",
	"to", " -", "打开 cfg.editor 以加载类型",
	"to", " <path>", "从 C 头文件加载类型",
	"tos", " <path>", "从解析的 Sdb 数据库加载类型",
	"toe", " [type.name]", "打开 cfg.editor 以编辑类型",
	"tos", " <path>", "从解析的 Sdb 数据库加载类型",
	"touch", " <file>", "在文件中创建或更新时间戳",
	NULL
};
...
static RCoreHelpMessage help_msg_tp = {
	"用法: tp[vx]", " <type> [...]", "打印类型",
	"tp", " <type> [addr|varname]", "将 <address> 的数据转换为 <type> 并打印 (XXX: 类型可以包含空格)",
	"tpv", " <type> [@addr]", "显示给定类型的偏移格式",
	"tpx", " <type> <hexpairs>", "显示指定字节序列的类型值 (XXX: 类型可以包含空格)",
	NULL
};

static RCoreHelpMessage help_msg_tc = {
	"用法: tc[...]", " [type]", "打印加载的类型",
	"tc", "", "以 C 输出格式列出所有加载的类型并换行",
	"tc", " [type.name]", "以 C 输出格式列出给定加载类型并换行",
	"tcd", "", "以 C 输出格式列出所有加载的类型而不换行",
	"tcc", "[?]", "管理调用约定类型",
	NULL
};
...
static RCoreHelpMessage help_msg_te = {
	"用法: te[...]", "", "类型枚举命令",
	"te", "", "列出所有加载的枚举",
	"te", " <enum>", "打印给定名称的枚举的所有值",
	"te", " <enum> <value>", "显示给定枚举编号的名称",
	"te-", "<enum>", "删除枚举类型定义",
	"teb", " <enum> <name>", "显示给定名称的匹配枚举位域",
	"tec", "", "以 C 输出格式列出所有加载的枚举并换行",
	"tec", " <name>", "以 C 输出格式列出给定加载的枚举并换行",
	"ted", "", "以 C 输出格式列出所有加载的枚举而不换行",
	"tej", "", "以 json 格式列出所有加载的枚举",
	"tej", " <enum>", "以 json 格式显示枚举",
	"test", " [-x,f,d] [path]", "测试可执行文件、文件或目录是否存在",
	NULL
};
...
static RCoreHelpMessage help_msg_tt = {
	"用法: tt[...]", "", "类型 typedef 命令",
	"tt", "", "列出所有加载的 typedefs",
	"tt", " <typename>", "显示给定类型别名的名称",
	"ttj", "", "以 json 格式显示 typename 和类型别名",
	"ttc", "<name>", "以 C 输出格式显示 typename 和类型别名",
	NULL
};

static RCoreHelpMessage help_msg_tl = {
	"用法: tl[...]", "[typename] [[=] address]", "类型链接命令",
	"tl", "", "列出所有链接。",
	"tll", "", "以可读格式列出所有链接。",
	"tllj", "", "以可读的 JSON 格式列出所有链接。",
	"tl", " [typename]", "将类型链接到当前地址。",
	"tl", " [typename] = [address]", "将类型链接到给定地址。",
	"tls", " [address]", "显示给定地址的链接。",
	"tl-*", "", "删除所有链接。",
	"tl-", " [address]", "删除给定地址的链接。",
	"tl*", "", "以 radare2 命令格式列出所有链接。",
	"tlj", "", "以 JSON 格式列出所有链接。",
	NULL
};

static RCoreHelpMessage help_msg_tn = {
	"用法:", "tn [-][0xaddr|symname]", " 管理无返回标记",
	"tn[a]", " 0x3000", "如果调用/跳转到此地址，则停止函数分析",
	"tn[n]", " sym.imp.exit", "与上面相同，但适用于标志/函数名称",
	"tnf", "", "与 afl,noret/eq/1 相同",
	"tn-", " 0x3000 sym.imp.exit ...", "移除一些无返回引用",
	"tn-*", "", "移除所有无返回引用",
	"tn", "", "列出它们所有",
	NULL
};
...
static RCoreHelpMessage help_msg_ts = {
	"用法: ts[...]", " [type]", "",
	"ts", "", "列出所有加载的结构",
	"ts", " [type]", "显示给定结构的 pf 格式字符串",
	"ts-", "[type]", "删除结构类型定义",
	"tsj", "", "以 json 格式列出所有加载的结构",
	"tsj", " [type]", "以 json 格式显示给定结构的 pf 格式字符串",
	"ts*", "", "显示所有加载结构的 pf.<name> 格式字符串",
	"ts*", " [type]", "显示给定结构的 pf.<name> 格式字符串",
	"tsc", "<name>", "以 C 输出格式列出所有/给定加载的结构并换行",
	"tsd", "", "以 C 输出格式列出所有加载的结构而不换行",
	"tss", " [type]", "显示结构的大小",
	NULL
};
...
static RCoreHelpMessage help_msg_tu = {
	"用法: tu[...]", "", "",
	"tu", "", "列出所有加载的联合",
	"tu", " [type]", "显示给定联合的 pf 格式字符串",
	"tuj", "", "以 json 格式列出所有加载的联合",
	"tuj", " [type]", "以 json 格式显示给定联合的 pf 格式字符串",
	"tu*", "", "显示所有加载联合的 pf.<name> 格式字符串",
	"tu*", " [type]", "显示给定联合的 pf.<name> 格式字符串",
	"tuc", "<name>", "以 C 输出格式列出所有/给定加载的联合并换行",
	"tud", "", "以 C 输出格式列出所有加载的联合而不换行",
	NULL
};
