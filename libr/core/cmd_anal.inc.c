...
static RCoreHelpMessage help_msg_af_plus = {
	"用法:", "af+", " [addr] ([name] ([type] [diff]))",
	"af+", "$$", "添加原始函数元素。参见 afb+ 为其添加基本块",
	"af+", "$$ main", "在当前偏移处添加名为 'main' 的新函数",
	NULL
};

static RCoreHelpMessage help_msg_aex = {
	"用法:", "aex", "[a] [9090]",
	"aex", " 90", "解码给定的十六进制对并执行",
	"aexa", " mov rax, 33", "汇编指令并执行",
	NULL
};

static RCoreHelpMessage help_msg_a = {
	"用法:", "a", "[abdefFghoprxstc] [...]",
	"a", "", "aai 的别名 - 分析信息",
	"a:", "[cmd]", "运行由分析插件实现的命令（类似于 : 用于 io）",
	"a*", "", "同 afl*;ah*;ax*",
	"aa", "[?]", "分析全部（函数 + 基本块）（aa0 避免子重命名）",
	"a8", " [hexpairs]", "分析字节",
	"ab", "[?]", "分析基本块",
	"ac", "[?]", "管理类",
	"aC", "[?]", "分析函数调用",
	"ad", "[?]", "分析数据跳板（进行中）（参见 'aod' 描述助记符）", // XXX 重命名为 at?
	"ad", " [from] [to]", "分析指向 (from-to) 的数据指针",
	"ae", "[?] [expr]", "分析操作码求值表达式（参见 ao）",
	"af", "[?]", "分析函数",
	"aF", "", "同上，但使用 anal.depth=1",
	"ag", "[?] [options]", "以各种格式绘制图形",
	"ah", "[?]", "分析提示（强制操作码大小，...）",
	"ai", " [addr]", "地址信息（显示权限、堆栈、堆等）",
	"aj", "", "同 a* 但输出为 json (aflj)",
	"aL", "[jq]", "列出所有 asm/anal 插件（参见 `e asm.arch=?` 和 `La[jq]`）",
	"an", "[?] [name]", "显示/重命名/创建当前指令中使用的任何变量/标志/函数",
	"ao", "[?] [len]", "分析操作码（或模拟执行）",
	"aO", "[?] [len]", "在 M 字节中分析 N 条指令",
	"ap", "", "查找当前偏移的函数前导码",
	"ar", "[?]", "类似于 'dr' 但用于 esil 虚拟机（寄存器）",
	"as", "[?] [num]", "使用 dbg.reg 分析系统调用",
	"av", "[?] [.]", "显示虚函数表",
	"avg", "[?] [.]", "管理全局变量",
	"ax", "[?]", "管理引用/交叉引用（另见 afx?）",
	NULL
...
static RCoreHelpMessage help_msg_afna = {
	"用法:", "afna", " # 构建函数名称并重命名当前偏移的函数。",
	"", "", "基于该函数内找到的标志或方法调用。",
	NULL
};

static RCoreHelpMessage help_msg_afu = {
	"用法:", "afu", "[addr]   # 调整大小并从当前地址分析函数直到 addr。",
	"afu", " 0x100004093", "从当前地址调整大小并分析函数直到 0x100004093",
	NULL
};

static RCoreHelpMessage help_msg_aae = {
	"用法:", "aae", "[pf] ([addr]) # 使用 esil 分析各种内容",
	"aae", " [size] ([addr])", "同 aepa@@@i - 通过导入标志名称定义分析锚点",
	"aae", "", "遵循 anal.{in,from,to} 并模拟所有可执行区域",
	"aaef", "", "使用 esil 模拟所有函数以找出计算出的引用（同 aef@@@F）",
	"aaep", "", "同 aepa@@@i - 通过导入标志名称定义分析锚点",
	"aaep", "a", "运行 'aep ret0@@@i' 然后 'aaep' - 所有未知导入被伪造为返回 0",
	"aaex", "", "线性模拟所有代码并仅注册计算出的交叉引用",
	NULL
};

static RCoreHelpMessage help_msg_aav = {
	"用法:", "aav", "[sat] # 查找引用特定段或映射的值",
	"aav", "", "查找绝对引用值（参见 aav0）",
	"aavq", "", "同 aav，但静默模式",
	"aav0", "", "查找绝对引用值（接受地址为零的映射）",
	"aavr", "[0]", "查找相对引用值（地址 + 4 字节有符号整数）",
	NULL
};
...
static RCoreHelpMessage help_msg_afm = {
	"用法:", "afm", "[name]   # 合并两个函数。",
	"afm", " sym.func.100003d74", "将当前函数合并到 0x100003d74",
	NULL
};

static RCoreHelpMessage help_msg_aF = {
	"用法:", "aF", " # 分析函数，但使用 anal.depth=1",
	"aF", "", "更多选项和信息请查看 af?。",
	NULL
};

static RCoreHelpMessage help_msg_an = {
	"用法:", "an", "[aj*] # 分析当前地址的名称（参见 fd 和 aan 命令）",
	"an", "[j*]", "显示标志/函数/符号名称（json 或 r2 命令格式）",
	"anf", "", "为当前函数建议名称（参见 anal.slow 和 'aan'）",
	"anfl", "", "列出自动命名猜测算法中使用的所有名称",
	"ano", "[?]", "显示或编辑当前函数的注解",
	NULL
};
...
static RCoreHelpMessage help_msg_ano = {
	...
	"ano-", "*", "移除当前文件的所有注解",
	"ano-", "$$", "移除当前函数的注解",
	"ano*", "", "以 ano= 命令格式转储所有注解",
	"ano=", "[:cmd]|[base64:]|text", "设置函数注解命令、纯文本或 base64",
	"anoe", "", "使用 cfg.editor 编辑注解",
	"anos", "", "显示当前函数注解",
	"anol", "", "显示函数注解的第一行（如果有）",
	NULL
};
...
static RCoreHelpMessage help_msg_a8 = {
	"用法:", "a8", "[hexpairs]   # 分析给定的字节数组",
	"a8 ", "5548", "分析 5548 字节数组",
	NULL
};
...
static RCoreHelpMessage help_msg_ap = {
	"用法:", "ap[?]", " # 分析当前偏移的函数前导码",
	"ap", "", "检查当前偏移是否包含函数前导码",
	"apl", "", "列出由架构插件定义的可用函数前导码",
	// "ap+", "bytes:mask:align", "添加新的函数前导码",
	"apt", "", "分析进程-线程（类似于 dpt 但用于 esil），列出线程",
	"apt ", "[id]", "选择线程（更改寄存器配置文件和 TLS 映射）",
	"apt+", "[map]", "使用给定的 TLS 映射 ID 创建线程并打印新线程 ID",
	"apt-", "[id]", "按 ID 删除线程",
	NULL
};
...
static RCoreHelpMessage help_msg_avg = {
	"用法:", "avg", " # 分析全局变量",
	"avg", "", "列出全局变量",
	"avg", " [type] [name]", "添加具有给定类型和名称的全局变量",
	"avg-", "", "删除全局变量",
	NULL
};

static RCoreHelpMessage help_msg_aC = {
	"用法:", "aC[fej] [addr-of-call]", " # 分析调用参数",
	"aCe", "", "使用 ESIL 模拟找出调用的参数（使用 'abpe'）",
	"aCf", "", "同 .aCe* $$ @@=`pdr~call`",
	NULL
};

static RCoreHelpMessage help_msg_aaf = {
	"用法:", "aaf[efrt?]", " # 分析所有函数，另见 'af' 和 'afna'",
	"aaf", "", "同 afr@@c:isq",
	"aafe", " ", "同 aef@@F",
	"aaff", "", "为每个函数设置标志",
	"aafr", " [len]", "连续函数分析 (e anal.hasnext=1;afr@@c:isq)",
	"aaft", "", "跨所有函数的递归类型匹配",
	"aafs", "", "单基本块函数分析",
	NULL
};
...
static RCoreHelpMessage help_msg_aaa = {
	"用法:", "aaa[a[a]]", " # 自动分析整个程序",
	"aaa", "", "执行更深入的分析，最常用",
	"aaaa", "", "同 aaa 但添加一堆实验性迭代",
	"aaaaa", "", "细化分析以在 aaaa 后找到更多函数",
	NULL
};

static RCoreHelpMessage help_msg_aa = {
	"用法:", "aa[0*?]", " # 另见 'af' 和 'afna'",
	"aa", " ", "别名 'af@@ sym.*;af@entry0;afva'", //;.afna @@ fcn.*'",
	"aaa", "[?]", "在 aa 后自动命名函数（参见 afna）",
	"aab", "", "跨 bin.sections.rx 的 abb",
	"aac", " [len]", "分析函数调用 (af @@ `pi len~call[1]`)",
	"aac*", " [len]", "标记函数调用而不执行完整分析",
	"aaci", "", "仅标记导入交叉引用",
	"aad", " [len]", "分析指向代码的数据引用",
	"aae", "[?] [len] ([addr])", "使用 ESIL 分析引用（可选到地址）",
	"aaef", "", "在所有函数中使用 ESIL 分析引用",
	"aaf", "[?][efrt] ", "分析所有函数与标志、类型匹配和连续的关系",
	"aaF", " [sym*]", "为匹配通配符的标志之间的所有空间设置 anal.in=block",
	"aaFa", " [sym*]", "同 aaF 但使用 af/a2f 而不是 af+/afb+（更慢但更准确）",
	"aai", "[j]", "显示所有分析参数的信息",
	"aaj", " ", "分析所有跳转",
	"aan", "[?][fgr]", "自动命名函数 (aang = golang, aanr = noreturn 传播, aanf = afna@@F)",
	"aao", "", "分析所有 objc 引用",
	"aap", "", "查找并分析函数前导码",
	"aar", "[?] [len]", "分析指令的 len 字节以查找引用",
	"aas", " [len]", "分析符号 (af @@= `isq~[0]`)",
	"aaS", "", "分析所有以 sym. 开头的标志 (af @@ sym.*)",
	"aat", " [fcn]", "分析所有/给定函数以将立即数转换为链接的结构偏移（参见 tl?）",
	"aaT", " [len]", "分析陷阱 sled 后的代码",
	"aau", " [len]", "列出未被函数覆盖的内存区域（大于 len 字节）",
	"aav", "[?] [sat]", "查找引用特定段或映射的值",
	"aaw", "", "分析所有元字 (Cd) 并为引用的指针添加 r. 命名标志",
	"aax", "[?]", "将所有交叉引用分析为函数",
	NULL
};
...
static RCoreHelpMessage help_msg_afls = {
	...
	// "afls", "", "TODO: 显示上次使用的顺序",
	"aflsa", "", "按地址排序（同 afls）",
	"aflsA", "", "按地址排序（与 alfsa 相反）",
	"aflsb", "", "按基本块数量排序",
	"aflsB", "", "按基本块数量排序（与 aflsb 相反）",
	"aflss", "", "按大小排序",
	"aflsS", "", "按大小排序（与 aflss 相反）",
	"aflsn", "", "按名称排序",
	"aflsN", "", "按名称排序（与 aflsn 相反）",
	NULL
};

static RCoreHelpMessage help_msg_afbs = {
	"用法:", "afbs", "[afbs] # 排序当前函数的基本块",
	// "afbs", "", "同 afbsa",
	"afbsa", "", "按地址排序（同 afbs）",
	"afbsA", "", "按地址排序（同 afbs） - 逆序",
	"afbss", "", "按大小排序",
	"afbsS", "", "按大小排序 - 逆序",
	// "afbsn", "", "按名称排序",
	// "afbsb", "", "按基本块数量排序",
...
static RCoreHelpMessage help_msg_aflx = {
	"用法:", "aflx", "[jv*] # 列出函数交叉引用",
	"aflx", "", "列出函数交叉引用（谁引用或调用当前函数）",
	"aflxj", "", "以 JSON 格式列出函数交叉引用",
	"aflxv", "", "列出带名称的函数交叉引用",
	"aflx*", "", "以 radare 命令格式列出函数交叉引用",
	NULL
};

static RCoreHelpMessage help_msg_ai = {
	"用法:", "ai", "[j*] [sz] # 分析/地址信息/导入",
	"ai", " @addr", "显示地址信息",
	"aia", "", "显示架构特定信息指令大小和对齐细节",
	"aii", " [namespace]", "全局导入（类似于 afii，但全局）",
	"aii", "-", "删除所有全局导入",
	"aij", " @addr", "以 JSON 格式显示地址信息",
	NULL
};

static RCoreHelpMessage help_msg_aar = {
	"用法:", "aar", "[j*] [sz] # 搜索并分析交叉引用",
	"aar", "", "分析当前段中的交叉引用",
	"aar", " [sz]", "分析从当前偏移开始的交叉引用，直到消耗 sz 字节",
	"aarr", "", "分析所有函数引用图以找到更多函数（实验性）",
	"aar*", " [sz]", "以 radare 命令格式列出找到的交叉引用",
	"aarj", " [sz]", "以 JSON 格式列出找到的交叉引用",
	NULL
};

static RCoreHelpMessage help_msg_ab = {
	"用法:", "ab", "# 分析基本块",
	"ab", " [addr]", "显示给定地址的基本块信息（同 abi/afbi）",
	"ab-", "[addr]", "删除给定地址的基本块",
	"ab.", "", "同: ab $$",
	"abi", "[?]", "afbi 的别名",
	"aba", "[j] [addr]", "分析基本块中的 esil 访问（参见 aea?）",
	"abb", " [length]", "分析 N 字节并提取基本块",
	"abc", "[-] [color]", "更改当前基本块的颜色（同 afbc，abc- 取消设置）",
	"abe", " [esil-expr]", "为基本块分配 esil 表达式（参见: aeb, dre, afbd）",
	"abf", " [addr]", "传入（来源）基本块的地址",
	"abj", " [addr]", "以 JSON 格式显示基本块信息",
	"abl", "[?] [.-cqj]", "列出所有基本块",
	"abo", "", "列出当前基本块的每个指令偏移",
	"abm", "[j]", "列出当前基本块的指令字节和掩码",
	"abp", "[?] [addr]", "从 $$ 到 `addr` 跟踪基本块路径",
	"abt", "[tag] ([color])", "无参数 = 显示当前跟踪标签，否则设置颜色",
	"abx", " [hexpair-bytes]", "分析 N 字节",
	NULL
};
...
static RCoreHelpMessage help_msg_abt = {
	...
	"abt", "", "列出此函数的回溯",
	"abt*", "", "以 r2 命令格式转储",
	"abt+", "[addr] [bt0,bt1,bt2,..]", "添加新回溯",
	"abt-", "[addr]", "从给定的 bt 中删除所有回溯",
	"abtn", "[tag] ([color])", "基本块跟踪标签编号：设置颜色和标签名",
	NULL
};

static RCoreHelpMessage help_msg_abl = {
	"用法:", "abl", "已分析的基本块列表",
	"abl", "", "列出所有程序范围内的已分析基本块",
	"abl,", " [table-query]", "使用表格渲染列表",
	"ablc", "", "统计注册的基本块数量",
	"ablj", "", "json 格式",
	"ablq", "", "简洁格式",
	NULL
...
static RCoreHelpMessage help_msg_abp = {
	"用法:", "abp", "[addr] [num] # 从当前偏移到 addr 查找 num 条路径",
	"abp", " [addr] [num]", "从当前偏移到 addr 查找 num 条路径",
	"abpf", " [addr]", "同 /gg 查找跨函数和基本块的两个地址之间的路径",
	"abpe", " [addr]", "从函数开始模拟到给定地址",
	"abpe*", " [addr]", "显示从函数开始模拟到给定地址的命令",
	"abpj", " [addr] [num]", "以 JSON 格式显示路径",
	NULL
};
...
static RCoreHelpMessage help_msg_ac = {
	"用法:", "ac", "分析类命令",
	"acl", "[j*]", "列出所有类",
	"acll", "[j] (class_name)", "列出所有或单个类的详细信息",
	"ac", " [class name]", "添加类",
	"ac-", " [class name]", "删除类",
	"acn", " [class name] [new class name]", "重命名类",
	"acv", " [class name] [addr] ([offset]) ([size])", "将虚函数表地址添加到类",
	"acvf", " [offset] ([class name])", "在虚函数表偏移处查找函数地址",
	"acv-", " [class name] [vtable id]", "按 ID 删除虚函数表（来自 acv [class name]）",
	"acb", " [class name]", "列出类的基类",
	"acb", " [class name] [base class name] ([offset])", "添加基类",
	"acb-", " [class name] [base class id]", "按 ID 删除基类（来自 acb [class name]）",
	"acm", " [class name] [method name] [offset] ([vtable offset])", "添加/编辑方法",
	"acm-", " [class name] [method name]", "删除方法",
	"acmn", " [class name] [method name] [new name]", "重命名方法",
	"acg", "", "以 ASCII 图形打印继承关系",
	"ac?", "", "显示此帮助",
	NULL
};
...
static RCoreHelpMessage help_msg_acolon = {
	"用法:", "a:", "[plugin-command]",
	"a:", "", "列出分析插件",
	"a:", "a2f", "运行与 'a2f' 分析插件关联的命令",
	NULL
};

static RCoreHelpMessage help_msg_ad = {
	"用法:", "ad", "[kt] [...]",
	"ad", " [N] [D]", "在 D 深度分析 N 个数据字",
	"ad4", " [N] [D]", "在 D 深度分析 N 个数据字 (asm.bits=32)",
	"ad8", " [N] [D]", "在 D 深度分析 N 个数据字 (asm.bits=64)",
	"adf", "", "分析函数中的数据（使用方式如 .adf @@=`afl~[0]`",
	"adfg", "", "分析函数间隙中的数据",
	"adt", "", "分析数据跳板（进行中）",
	"adk", "", "分析数据类型（代码、文本、数据、无效等）",
	NULL
};

static RCoreHelpMessage help_msg_aes = {
	"用法:", "aes[pbosu]", "esil 单步实用程序",
	"aesp", " [X] [N]", "从偏移 X 开始评估 N 条指令",
	"aesb", "", "后退一步",
	"aeso", " ", "步过",
	"aesou", " [addr]", "步过直到给定地址",
	"aess", " ", "步跳（在 CALL 情况下，只是跳过，而不是步入）",
	"aesu", " [addr]", "步进直到给定地址",
	"aesue", " [esil]", "步进直到 esil 表达式匹配",
	"aesuo", " [optype]", "步进直到给定操作码类型",
	"aesB", " [addr] [N] @ [from-addr]", "每 N 条指令步过一次",
	NULL
};

static RCoreHelpMessage help_msg_aei = {
	"用法:", "aei", "[smp] [...]",
	"aei", "", "初始化 ESIL VM 状态 (aei- 取消初始化)",
	"aeis", " argc [argv] [envp]", "初始化入口点堆栈环境",
	"aeim", "[?] [addr] [size] [name]", "初始化 ESIL VM 堆栈 (aeim- 移除)",
	"aeip", "", "将 ESIL 程序计数器初始化为 curseek",
	NULL
};

static RCoreHelpMessage help_msg_ae = {
	"用法:", "ae[idesr?] [arg]", "ESIL 代码模拟",
	"ae", " [expr]", "评估 ESIL 表达式",
	"ae!", " [file]", "将 esil 文件编译为 esil 表达式",
	"ae?", "", "显示此帮助",
	"ae??", "", "显示 ESIL 帮助",
	"ae???", "", "列出 ESIL 操作",
	"aea", "[f] [count]", "分析 n 条 esil 指令的访问（寄存器、内存等）",
	"aeA", "[f] [count]", "分析 n 字节的 esil 访问（寄存器、内存等）",
	"aeb", " ([addr])", "模拟当前或给定地址的块",
	"aeC", "[arg0 arg1..] @ addr", "在 esil 中进行应用调用",
	"aec", "[?]", "继续直到 ^C",
	"aef", " [addr]", "模拟函数",
	"aefa", " [addr]", "模拟函数以找出给定或当前偏移的参数",
	"aeg", " [expr]", "esil 数据流图",
	"aegf", " [expr] [register]", "esil 数据流图过滤器",
	"aei", "[?]", "初始化 ESIL VM 状态 (aei- 取消初始化)",
	"aek", "[?] [query]", "在 ESIL.info 上执行 sdb 查询",
	"aeL", "[?][-] [name]", "列出 ESIL 插件",
	"aep", "[?] [addr]", "管理 esil 锚点钩子（参见 'e cmd.esil.pin'）",
	"aepc", " [addr]", "将 esil PC 更改为此地址",
	"aer", "[?] [..]", "像 'ar' 或 'dr' 一样处理 ESIL 寄存器",
	"aes", "[?]", "执行模拟调试器单步",
	"aet", "[?][s]", "esil 跟踪列表和会话管理（需要 aeim）",
	"aev", " [esil]", "给定表达式或当前指令的可视化 esil 调试器",
	"aex", " [hex]", "评估操作码表达式",
	NULL
};
...
static RCoreHelpMessage help_detail_ae = {
	...
	"ROUND", "", "浮点数四舍五入",
	"SQRT", "", "浮点数平方根",
	"POP", "", "丢弃 esil 堆栈中的最后一个元素",
	"DUP", "", "复制堆栈中的最后一个值",
	"NUM", "", "将堆栈中的最后一项评估为数字",
	"SWAP", "", "交换堆栈中的最后两个值",
	"TRAP", "", "停止执行",
	"BITS", "", "16,BITS  # 更改位数，对 arm/thumb 有用",
	"TODO", "", "指令尚未 esil 化",
	"STACK", "", "显示堆栈内容",
	"CLEAR", "", "清除 esil 堆栈",
	"REPEAT", "", "重复 n 次",
...
static RCoreHelpMessage help_msg_aea = {
	"示例:", "aea", " 显示范围内使用的寄存器和内存访问",
	"aea", "  [ops]", "显示 N 条指令中使用的寄存器/内存访问",
	"aeae", "  [esil]", "显示给定表达式中使用的寄存器/内存访问",
	"aea*", " [ops]", "为内存访问创建 mem.* 标志",
	"aeab", "", "显示当前基本块中使用的寄存器",
	"aeaf", "", "显示当前函数中使用的寄存器",
	"aear", " [ops]", "显示 N 条指令中读取的寄存器",
	"aeaw", " [ops]", "显示 N 条指令中写入的寄存器",
	"aean", " [ops]", "显示 N 条指令中未写入的寄存器",
	"aeaj", " [ops]", "以 JSON 格式显示 aea 输出",
	"aeA", "  [len]", "显示 N 字节中使用的寄存器（与 'aea' 相同的子命令）",
	"图例:", "", "",
	"I", "", "输入寄存器（在设置之前读取）",
	"A", "", "所有访问的寄存器",
	"R", "", "读取的寄存器值",
	...
	"@R", "", "内存读取",
	"@W", "", "内存写入",
	"注意:", "", "使用 PIC 的 mem{reads,writes} 仅获取偏移量",
	NULL
};
...
static RCoreHelpMessage help_msg_aec = {
	...
	"aecc", "", "继续直到调用",
	"aecu", "[addr]", "继续直到地址",
	"aecue", "[addr]", "继续直到 esil 表达式",
	NULL
};

static RCoreHelpMessage help_msg_aeC = {
	"示例:", "aeC", " arg0 arg1 ... @ calladdr",
	"aeC", " 1 2 @ sym._add", "调用 sym._add(1,2)",
	NULL
};
...
static RCoreHelpMessage help_msg_aep = {
	"用法:", "aep[-*c] ", " [...] 管理 esil 锚点，运行 r2 命令而不是 esil",
	"aepc", " [addr]", "更改 esil 的程序计数器",
	"aep*", "", "以 r2 命令列出锚点",
	"aep-", "*", "移除所有锚点",
	"aep-", "[addr]", "移除锚点",
	"aep-", "[name]", "移除锚点命令",
	"aepa", " ([addr])", "通过标志名称在当前或给定地址自动设置锚点（参见 aaep）",
	"aep", " [name] @ [addr]", "设置锚点",
	"aep ", "memcpy=wf `dr?A1` `dr?A2` @r:A0", "覆盖此锚点名称的 esil.cmd.pin",
	"aep ", "soft.show.r9=dr?r9", "设置 'soft.' esil 锚点，仅在反汇编循环中执行",
	"aep", "", "列出锚点",
	"aep.", "", "显示当前地址的锚点名称（如果有）",
	"aepk", " [query]", "查询锚点的 sdb",
	NULL
};

static RCoreHelpMessage help_msg_aek = {
	"用法:", "aek ", " [...]",
	"aek", "", "转储 esil.stats 数据库内容",
	"aek ", "sdb.query", "在 esil.stats db 上评估 sdb 查询",
	"aek-", "", "清除 esil.stats sdb 实例",
	NULL
};
...
static RCoreHelpMessage help_msg_af = {
	"用法:", "af", "",
	"af", " ([name]) ([addr])", "分析函数（从 addr 或 $$ 开始）",
	"af+", " addr name [type] [diff]", "手动创建函数（需要 afb+）",
	"af-", " [addr]", "清除所有函数分析数据（或 addr 处的函数）",
	"afa", "", "分析调用中的函数参数（afal 遵循 dbg.funcarg）",
	"afB", " 16", "将当前函数设置为 thumb（更改 asm.bits）",
	"afb", "[?] [addr]", "列出给定函数的基本块",
	"afc", "[?] type @[addr]", "设置函数的调用约定",
	"afC", "[?] ([addr])@[addr]", "计算周期数 (afC) 或圈复杂度 (afCc)",
	"afd", "[addr]", "显示函数 + 给定偏移的增量",
	"afe", "", "分析函数入口点位置",
	"afF", "[1|0|]", "折叠/展开/切换",
	"afi", "[?] [addr|fcn.name]", "显示函数信息（详细 afl）",
	"afj", " [tableaddr] [elem_sz] [count] [seg]", "分析函数跳转表（为每个元素添加 seg）",
	"afl", "[?] [ls*] [fcn name]", "列出函数（地址、大小、基本块、名称）（参见 afll）",
	"afm", " name", "合并两个函数",
	"afM", " name", "打印函数映射",
	"afn", "[?] name [addr]", "重命名地址处函数的名称（同时更改标志）",
	"afna", "", "为当前偏移建议自动名称",
	"afo", "[?j] [fcn.name]", "显示函数名称的地址或当前偏移",
	"afr", " ([name]) ([addr])", "递归分析函数",
	"afs", "[?] ([fcnsign])", "获取/设置当前地址的函数签名（afs! 使用 cfg.editor）",
	"afS", "[stack_size]", "设置当前地址函数的堆栈帧大小",
//	"afsr", " [function_name] [new_type]", "更改给定函数的类型",
	"aft", "[?]", "类型匹配，类型传播",
	"afu", " addr", "调整大小并从当前地址分析函数直到 addr",
	"afv", "[?]", "操作函数中的参数、寄存器和变量",
	"afx", "[m]", "列出函数引用",
	NULL
};
...
static RCoreHelpMessage help_msg_afb = {
	...
	"afb.", " [addr]", "显示当前基本块的信息",
	"afb,", "", "以表格形式显示当前函数的基本块",
	"afb=", "", "以 ASCII 艺术条显示基本块区域",
	//"afb+", " fcnA bbA sz [j] [f] ([t]( [d]))", "向函数 @ fcnaddr 添加基本块",
	"afb+", " fcn_at bbat bbsz [jump] [fail] ([diff])", "手动添加基本块",
	"afba", "[!]", "以分析顺序列出当前偏移的基本块（实验性，参见 afla）",
	"afbc", "[-] [color] ([addr])", "为基本块着色（同 'abc'，afbc- 取消设置）",
	"afbd", "", "按顺序列出函数基本块依赖列表并设置 abe 值",
	"afbe", " bbfrom bbto", "为 switch-case 添加基本块边",
	"afbF", "([0|1])", "切换基本块的 '折叠' 属性", // 为什么不是小写?
	"afbi", "[j]", "打印当前基本块信息",
	"afbj", " [addr]", "以 json 格式显示基本块信息",
	"afbr", "[?]", "显示离开函数的指令地址",
	"afbo", "", "列出函数中每个基本块的每条指令地址（参见 abo）",
	"afB", " [bits]", "定义给定函数的 asm.bits",
	NULL
};

static RCoreHelpMessage help_msg_afc = {
	"用法:", "afc[agl?]", "# 另见 tcc 命令管理所有调用约定",
	"afc", " ccname", "手动设置当前函数的调用约定",
	"afc", "", "显示默认函数调用约定（同 tcc）",
	"afcr", "[j]", "显示当前函数的寄存器使用情况",
	"afcf", "[j] [name]", "打印返回类型 函数(arg1, arg2...)，参见 afij",
	"afci", " [name]", "关于当前调用约定的信息",
	"afcj", "", "以 JSON 格式显示当前调用约定信息",
	"afck", "", "列出已加载调用约定的 SDB 详细信息",
	"afcl", "", "列出所有可用的调用约定",
	"afcll", "", "显示所有调用约定及其定义",
	"afco", " path", "从给定路径打开调用约定 SDB 配置文件",
	"afcR", "", "使用调用约定顺序进行寄存器伸缩",
	NULL
};
...
static RCoreHelpMessage help_msg_afi_fields = {
	...
	"addr", "", "函数入口点的绝对地址",
	"name", "", "函数名称",
	"size", "", "函数大小", // 重命名为 bbsize
	"realsz", "", "线性大小 ((maxbb+maxxbsz) - minbb)", // 重命名为 linearsize
	"stackframe", "", "堆栈帧大小",
	"callconv", "", "调用约定（ms, cdecl, ... 参见 afcl）",
	"cyclo-cost", "", "基本块中所有指令的 CPU 周期成本总和",
	"cyclo-complexity", "", "CC = 边 - 节点 + (2 * 结束节点)",
	"bits", "", "arch.bits 值",
	"type", "", "与差异分析相关",
	"num-bbs", "", "基本块数量",
	"num-instrs", "", "指令数量",
	...
	"minaddr", "", "最小地址",
	"maxaddr", "", "最大地址",
	"islineal", "", "基本块是否在内存中线性分配",
	"end-bbs", "", "返回的基本块数量",
	"maxbbins", "", "任何单个基本块中的最大指令数",
	"midbbins", "", "每个基本块的平均指令数",
	"ratbbins", "", "基本块与指令之间的比率",
	"noreturn", "", "如果最终调用无返回函数如 exit 或 assert 则为 true",
	"recursive", "", "如果函数调用自身则为 true",
	"in-degree", "", "调用此函数的函数数量（调用图中的传入边）",
	"out-degree", "", "此函数调用的函数数量（0 = 不调用其他函数）",
	"locals", "", "局部变量数量",
	"args", "", "函数参数",
	NULL
};
...
static RCoreHelpMessage help_msg_afi = {
	...
	"afi", "", "显示函数信息",
	"afi", "[.j*]", "函数、变量和参数",
	"afii", "[?][-][import]", "显示/添加/删除函数中使用的导入（参见 aii）",
	"afil", "", "详细函数信息",
	"afip", "", "显示函数是否为纯函数",
	"afiq", "", "显示函数的少量信息",
	"afis", "", "显示函数统计（操作码、元数据）",
...
static RCoreHelpMessage help_msg_afl = {
	"用法:", "afl", " 列出所有函数",
	"afl", "", "列出函数",
	"afl.", "", "显示当前偏移的函数（参见 afi.）",
	"afl,", " [query]", "以表格格式列出函数",
	"afl+", "", "显示所有函数大小的总和",
	"afl*", "", "以 r2 命令重建所有函数",
	"afl=", "", "以 ASCII 艺术条显示函数范围",
	"afla", "[j]", "反向调用顺序（对 afna 和 noret 有用，另见 afba）",
	"aflc", "", "函数计数",
	"aflj", "", "以 json 格式列出函数",
	"afll", " [column]", "以详细模式列出函数（按列名排序）",
	"afllj", "", "以详细模式列出函数（aflj 的别名）",
	"aflm", "[?]", "以 makefile 样式列出函数 (af@@=`aflm~0x`)",
	"afln", "[?]", "列出所有函数名称",
	"aflq", "", "以简洁模式列出函数",
	"aflqj", "", "以 json 简洁模式列出函数",
	"afls", "[?asn]", "按地址、大小或名称排序函数列表",
	"aflx", "[?*jv]", "列出函数交叉引用（谁引用或调用当前函数）",
	NULL
};
...
static RCoreHelpMessage help_msg_afii = {
	"用法:", "afii", " 管理当前函数使用的导入（全局导入参见 aii）。从反汇编中修剪这些字符串",
	"afii", "", "列出当前函数使用的导入",
	"afii", " sym.imp", "从所有调用中修剪 'sym.imp' 前缀",
	"afii", "-sym.imp", "从当前函数中移除此导入",
	NULL
};
...
static RCoreHelpMessage help_msg_afll = {
	...
	"nbbs", "", "基本块数量",
	"edges", "", "基本块之间的边数",
	"cc", "", "圈复杂度（cc = 边 - 块 + 2 * 退出块）",
	"cost", "", "圈成本",
	"min addr", "", "最小地址（最低的基本块）",
	"range", "", "函数大小",
	"max addr", "", "最大地址（最后一个基本块 + 大小）",
	"calls", "", "调用者函数数量",
	"locals", "", "局部变量数量",
	"args", "", "函数参数数量",
	"xref", "", "交叉引用数量",
...
static RCoreHelpMessage help_msg_afn = {
	"用法:", "afn[sa]", " 分析函数名称",
	"afn", " [name]", "重命名函数（名称可以以 base64: 开头）",
	"afn.", "", "同无参数的 afn。显示当前偏移的函数名称",
	"afn*", "", "显示设置函数签名、标志和函数名称的 r2 命令",
	"afna", "[j]", "为当前偏移构建函数名称",
	"afns", "[j]", "列出与当前函数关联的所有字符串",
	NULL
};
...
static RCoreHelpMessage help_msg_afs = {
	"用法:", "afs[r]", " 分析函数签名",
	"afs", " ([fcnsign])", "获取/设置当前地址的函数签名（afs! 使用 cfg.editor）",
	"afsq", "", "同 afs 但没有错误消息（非当前偏移参见 afsQ）",
	"afs!", "", "使用 cfg.editor 编辑当前函数签名",
	"afs*", " ([signame])", "以标志形式获取函数签名",
	"afsj", " ([signame])", "以 JSON 格式获取函数签名",
	"afsr", " [function_name] [new_type]", "更改给定函数的类型",
	"afsv", "[j]", "从当前寄存器和堆栈状态填充值打印函数签名",
	NULL
};
...
static RCoreHelpMessage help_msg_afv = {
	"用法:", "afv[rbs]", " 函数变量操作",
	"afv*", "", "输出添加参数/局部变量到标志空间的 r2 命令",
	"afv-", "([name])", "移除所有或给定变量",
	"afv=", "", "列出函数变量和参数及反汇编引用",
	"afva", "", "分析函数参数/局部变量",
	"afvb", "[?]", "操作基于 bp 的参数/局部变量",
	"afvd", " name", "输出在调试器中显示参数/局部变量值的 r2 命令",
	"afvf", "", "显示基于 BP 的相对堆栈帧变量",
	"afvn", " [new_name] ([old_name])", "重命名参数/局部变量",
	"afvr", "[?]", "操作基于寄存器的参数",
	"afvR", " [varname]", "列出变量被访问的地址（读取）",
	"afvs", "[?]", "操作基于 sp 的参数/局部变量",
	"afvt", " [name] ([type])", "显示或更改给定局部变量/参数的类型",
	"afvW", " [varname]", "列出变量被访问的地址（写入）",
	"afvx", "", "显示函数变量交叉引用（同 afvR+afvW）",
	NULL
};

static RCoreHelpMessage help_msg_aeim = {
	"用法:", "aeim", " [addr] [size] [name] - 初始化 ESIL VM 堆栈",
	"aeim", "", "使用 esil.stack.* 评估的默认值初始化 esil 内存",
	"aeim", " 0x10000", "同 aeim@e:esil.stack.addr=0x10000",
	"aeim", " 0x10000 2M mystack", "为新的 2MB 堆栈命名",
	NULL
};

static RCoreHelpMessage help_msg_afvb = {
	"用法:", "afvb", " [idx] [name] ([type])",
	"afvb", "", "列出基于基指针的参数、局部变量",
	"afvb*", "", "同 afvb 但输出为 r2 命令",
	"afvb", " [idx] [name] ([type])", "定义基于基指针的参数、局部变量",
	"afvbj", "", "以 JSON 格式返回基于基指针的参数、局部变量列表",
	"afvb-", " [name]", "删除给定名称的参数/局部变量",
	"afvbg", " [idx] [addr]", "定义变量获取引用",
	"afvbs", " [idx] [addr]", "定义变量设置引用",
	NULL
};

static RCoreHelpMessage help_msg_afvr = {
	"用法:", "afvr", " [reg] [type] [name]",
	"afvr", "", "列出基于寄存器的参数",
	"afvr*", "", "同 afvr 但输出为 r2 命令",
	"afvr", " [reg] [name] ([type])", "定义寄存器参数",
	"afvrj", "", "以 JSON 格式返回寄存器参数列表",
	"afvr-", " [name]", "删除给定索引的寄存器参数",
	"afvrg", " [reg] [addr]", "定义参数获取引用",
	"afvrs", " [reg] [addr]", "定义参数设置引用",
	NULL
};

static RCoreHelpMessage help_msg_afvs = {
	"用法:", "afvs", " [idx] [type] [name]",
	"afvs", "", "列出基于堆栈的参数和局部变量",
	"afvs*", "", "同 afvs 但输出为 r2 命令",
	"afvs", " [idx] [name] [type]", "定义基于堆栈的参数、局部变量",
	"afvsj", "", "以 JSON 格式返回基于堆栈的参数和局部变量列表",
	"afvs-", " [name]", "删除给定名称的基于堆栈的参数或局部变量",
	"afvsg", " [idx] [addr]", "定义变量获取引用",
	"afvss", " [idx] [addr]", "定义变量设置引用",
	NULL
};

static RCoreHelpMessage help_msg_ag = {
	"用法:", "ag<graphtype><format> [addr]", "",
	"图形命令:", "", "",
	"aga", "[format]", "数据引用图",
	"agA", "[format]", "全局数据引用图",
	"agc", "[format]", "函数调用图",
	"agC", "[format]", "全局调用图",
	"agd", "[format] [fcn addr]", "差异图",
	"agD", "[format]", "函数支配图",
	"agf", "[format]", "基本块函数图",
	"agi", "[format]", "导入图",
	"agr", "[format]", "引用图",
	...
	"agt", "[format]", "树状图",
	"ag-", "", "清除自定义图",
	"agn", "[?] title body", "向自定义图添加节点",
	"age", "[?] title1 title2", "向自定义图添加边",
	"", "", "",
	"输出格式:", "", "",
	"<blank>", "", "ascii 艺术",
	"*", "", "r2 命令",
	...
	"t", "", "微型 ASCII 艺术",
	"v", "", "交互式 ASCII 艺术",
	"w", " [path]", "写入路径或显示图形图像（参见 graph.gv.format）",
	NULL
};

static RCoreHelpMessage help_msg_age = {
	"用法:", "age [title1] [title2]", "",
	"示例:", "", "",
	"age", " title1 title2", "添加从标题为 \"title1\" 的节点到标题为 \"title2\" 的节点的边",
	"age", " \"title1 with spaces\" title2", "添加从节点 \"title1 with spaces\" 到节点 \"title2\" 的边",
	"age-", " title1 title2", "移除从标题为 \"title1\" 的节点到标题为 \"title2\" 的节点的边",
	"ageh", "", "列出所有高亮边",
	"ageh", " nodeA nodeB", "高亮 nodeA 和 nodeB 之间的边",
	"ageh-", " nodeA nodeB", "取消高亮 nodeA 和 nodeB 之间的边",
	"age?", "", "显示此帮助",
	NULL
};
...
static RCoreHelpMessage help_msg_agn = {
	"用法:", "agn [title] [body]", "",
	"示例:", "", "",
	"agn", " title1 body1", "添加标题为 \"title1\" 且内容为 \"body1\" 的节点",
	"agn", " \"title with space\" \"body with space\"", "添加标题和内容中包含空格的节点",
	"agn", " title1 base64:Ym9keTE=", "添加内容以 base64 指定的节点",
	"agn-", " title1", "移除标题为 \"title1\" 的节点",
	"agn?", "", "显示此帮助",
	NULL
};
...
static RCoreHelpMessage help_msg_ah = {
	...
	"ah.", "", "从当前偏移以人类可读格式列出提示",
	"ah-", "", "移除所有提示",
	"ah-", " offset [size]", "移除给定偏移的提示",
	"ah*", " offset", "以 radare 命令格式列出提示",
	"aha", " ppc @ 0x42", "强制所有地址 >= 0x42 或直到下一个提示使用 ppc 架构",
	"aha", " 0 @ 0x84", "禁用所有地址 >= 0x84 或直到下一个提示的架构提示效果",
	"ahb", "[-*] [8,16,32,64] @ 0x42", "获取/设置给定地址及以后的 asm.bits",
	"ahc", " 0x804804", "覆盖调用/跳转地址",
	"ahd", " foo a0,33", "替换操作码字符串",
	"ahe", " 3,eax,+=", "设置虚拟机分析字符串",
	"ahf", " 0x804840", "覆盖调用的回退地址",
	"ahF", " 0x10", "在当前偏移设置堆栈帧大小",
	"ahh", " 0x804840", "在反汇编中高亮此地址偏移",
	"ahi", "[?] 10", "定义立即数的数值基数（2, 8, 10, 10u, 16, i, p, S, s）",
	"ahj", "", "以 JSON 格式列出提示",
	"aho", " call", "更改操作码类型（参见 aho?）",
	"ahp", " addr", "设置指针提示",
	"ahr", " val", "设置函数返回值的提示",
	"ahs", " 4", "设置操作码大小=4",
	"ahS", " jz", "设置此操作码的 asm.syntax=jz",
	"aht", "[?][s] <type>", "将立即数标记为类型偏移（已弃用，移至 \"aho\"）",
	"ahv", " val", "更改操作码的 val 字段（对设置 jmp rax 中的跳转表大小有用）",
	NULL
};

static RCoreHelpMessage help_msg_ahs = {
	"用法:", "ahs [size] [@ addr]", " 定义操作码大小提示",
	"ahs", " 16", "提示分析使指令大小为 16 字节",
	"ahs-", "", "取消当前偏移的指令大小提示",
	"ahs-", "*", "取消当前偏移的指令大小提示",
	"ahs*", "", "以 r2 命令格式显示所有指令大小提示",
	NULL
};

static RCoreHelpMessage help_msg_aho = {
	"用法:", "aho [optype] [@ addr]", " 定义操作码类型提示",
	"aho", " nop", "将当前地址的操作码类型更改为被视为 NOP",
	"aho", "", "显示当前操作码提示（如果有）",
	"aho-", "$$", "删除当前偏移的提示",
	"aho*", "", "同上，但以 r2 命令语法",
	NULL
...
static RCoreHelpMessage help_msg_ahb = {
	"用法:", "ahb [8|16|32|64] [@ addr]", " 在给定地址定义 asm.bits 提示",
	"ahb", " 16", "在给定地址设置 asm.bits=16",
	"ahb", "", "获取给定地址（当前搜索）使用的 asm.bits",
	"ahb", "-$$", "删除给定地址的所有提示",
	"ahb*", "", "以 r2 命令显示定义的位提示",
	NULL
};
...
static RCoreHelpMessage help_msg_ahr = {
	"用法:", "ahr addr", " 将指令设置为返回类型（类似于 'aho ret'?）",
	"ahr", " $$", "当前指令可能被视为函数的结束",
	NULL
};

static RCoreHelpMessage help_msg_ahi = {
	"用法:", "ahi [2|8|10|10u|16|bodhipSs] [@ offset]", " 定义数值基数",
	"ahi", " <base>", "设置数值基数（2, 8, 10, 16）",
	"ahi", " 10|d", "设置为有符号十进制（10），符号位应取决于接收者大小",
	"ahi", " 10u|du", "设置为无符号十进制（11）",
	"ahi", " 31", "31位小整数，忽略低位（>>=1）",
	"ahi", " b", "设置为二进制（2）",
	"ahi", " o", "设置为八进制（8）",
	"ahi", " h", "设置为十六进制（16）",
	"ahi", " i", "设置为 IP 地址（32）",
	"ahi", " p", "设置为 htons(端口)（3）",
	"ahi", " S", "设置为系统调用（80）",
	"ahi", " s", "设置为字符串（1）",
	"ahi1", " 10", "将参数 1 的基数设置为 10（同 ahi1 d）",
	NULL
};

static RCoreHelpMessage help_msg_aht = {
	"用法:", "aht[s] [addr|type]", "将立即数标记为类型偏移（已移至 aho）",
	"ahts", " <offset>", "列出所有匹配的结构偏移",
	"aht", " <struct.member>", "将立即数更改为结构偏移",
	"aht?", "", "显示此帮助",
	NULL
};
...
static RCoreHelpMessage help_msg_aot = {
	"用法:", "aot[l]", "列出操作码类型",
	"aot", "", "显示当前指令的类型",
	"aotl", "", "列出所有可能的操作码类型（参见 /atl）",
	NULL
};

static RCoreHelpMessage help_msg_aom = {
	"用法:", "aom[ljd] [arg]", "列出操作码助记符",
	"aom", "", "显示指令助记符",
	"aom.", "", "显示当前地址的指令助记符",
	"aoml", "", "列出所有助记符",
	"aomj", "", "以 json 格式列出（TODO：也添加指令描述？）",
	"aomd", "", "带指令描述的详细助记符列表",
	NULL
};

static RCoreHelpMessage help_msg_aob = {
	"用法:", "aob[mvj] [hex]", "解码已分析操作码的每一位",
	"aob", "", "显示指令中每一位的含义",
	"aob", " cd80", "解码 'int 0x80' 并分析其位",
	"aobj", "", "操作码解码信息的 json 格式",
	"aobv", "", "视觉吸引力的表示",
	"aobV", "", "顶部带有位流的视觉吸引力的表示",
	NULL
};

static RCoreHelpMessage help_msg_ao = {
	"用法:", "ao[e?] [len]", "分析操作码",
	"ao", " 5", "显示 5 个操作码的操作码分析",
	"ao*", "", "以 r 命令显示操作码",
	"aob", "[?mvj] ([hex])", "分析当前操作码中每一位的含义",
	"aoc", " [cycles]", "分析哪些操作可以在 [cycles] 内执行",
	"aod", " [mnemonic]", "asm.arch 的指令助记符描述",
	"aoda", "", "显示所有助记符描述",
	"aoe", " N", "显示 N 个操作码的 esil 形式",
	"aoem", " N", "显示 N 个操作码的 esil 模拟中的内存引用",
	"aoef", " expr", "按给定输出过滤操作码的 esil 表达式",
	"aoeq", " N", "仅显示 N 个操作码的 esil 表达式",
	"aoj", " N", "以 JSON 格式显示 N 个操作码的操作码分析信息",
	"aom", "[?] [id]", "列出当前或当前架构的所有助记符",
	"aor", " [N]", "运行 N 条 esil 指令 + esil.dumpstack",
	"aos", " N", "显示 N 个操作码的大小",
	"aot", "[?]", "列出所有操作码类型",
	NULL
};
...
static RCoreHelpMessage help_msg_ar = {
	"用法: ar", "", "# 分析寄存器",
	"ar", "", "显示 'gpr' 寄存器",
	"ar.", ">$snapshot", "显示将寄存器值设置为当前状态的 r2 命令",
	"ar,", "", "以表格格式显示寄存器（参见 dr,）",
	".ar*", "", "将寄存器值导入为标志",
	".ar-", "", "取消所有寄存器的标志",
	"ar0", "", "将寄存器区域重置为 0",
	"ara", "[?]", "管理寄存器区域",
	"arj", "", "以 JSON 格式显示 'gpr' 寄存器",
	"arA", "[?]", "显示函数调用参数的值（A0, A1, A2, ..）",
	"ar", " 16", "显示 16 位寄存器",
	"ar", " 32", "显示 32 位寄存器",
	"ar", " all", "显示所有位寄存器",
	"ar", " <type>", "显示给定类型的所有寄存器",
	"arC", "", "显示寄存器配置文件注释",
	"arr", "", "显示寄存器引用（伸缩）",
	"arrj", "", "以 JSON 格式显示寄存器引用（伸缩）",
	"ar=", "([size])(:[regs])", "以列显示寄存器值",
	"ar?", " <reg>", "显示寄存器值",
	"arb", " <type>", "显示给定区域的十六进制转储",
	"arc", "[cq=] <name>", "条件标志寄存器",
	"arcc", "", "从寄存器配置文件派生调用约定",
	"ard", " <name>", "仅显示不同的寄存器",
	"arn", " <regalias>", "获取 pc,sp,bp,a0-3,zf,cf,of,sg 的寄存器名称",
	"aro", "", "显示旧（先前）的寄存器值",
	"arp", "[?] <file>", "从文件加载寄存器配置文件",
	"ars", "", "堆栈寄存器状态",
	"arS", "", "显示寄存器配置文件的大小",
	"art", "", "列出所有寄存器类型",
	"arw", " <hexnum>", "设置寄存器区域的内容",
	NULL
};

static RCoreHelpMessage help_msg_ara = {
	"用法:", "ara[+-s]", "寄存器区域 推送/弹出/交换",
	"ara", "", "显示所有分配的寄存器区域",
	"ara", "+", "为每种类型推送新的寄存器区域",
	"ara", "-", "弹出最后一个寄存器区域",
	"aras", "", "交换最后两个寄存器区域",
...
static RCoreHelpMessage help_msg_as = {
	"用法: as[ljk?]", "", "系统调用名称 <-> 编号实用程序",
	"as", "", "显示当前系统调用和参数",
	"as", " 4", "基于 asm.os 和当前寄存器/内存显示系统调用 4",
	"asc[a]", " 4", "以 .asm 或 .h 格式转储系统调用信息",
	"asj", "", "以 JSON 格式列出系统调用",
	"asl", "", "按 asm.os 和 asm.arch 列出系统调用",
	"asl", " close", "返回 close 的系统调用编号",
	"asl", " 4", "返回系统调用编号 4 的名称",
	"ask", " [query]", "执行 syscall/ 查询",
	NULL
};

static RCoreHelpMessage help_msg_av = {
	"用法:", "av[?jr*]", " C++ 虚函数表和 RTTI",
	"av", "", "在数据段中搜索虚函数表并显示结果",
	"avj", "", "同 av，但输出为 json",
	"av*", "", "同 av，但输出为 r2 命令",
	"avr", "[j@addr]", "尝试在虚函数表地址解析 RTTI（参见 anal.cxxabi）",
	"avra", "[j]", "搜索虚函数表并尝试在每个表解析 RTTI",
	"avrr", "", "从所有可找到的 RTTI 恢复类信息（参见 ac）",
	"avrD", " [classname]", "从 RTTI 反混淆类名",
	NULL
};

static RCoreHelpMessage help_msg_ax = {
	"用法:", "ax[?d-l*]", " # 另见 'afx?'",
	"ax", " addr [at]", "添加指向 addr 的代码引用（从 curseek）",
	"ax", "", "列出引用",
	"ax*", "", "输出 radare 命令",
	"ax-", " [at]", "清除所有引用/来自 addr 的引用",
	"ax-*", "", "清除所有引用/引用",
	"ax.", " [addr]", "查找从此地址和到此地址的数据/代码引用",
	"axc", " addr [at]", "添加通用代码引用",
	"axC", " addr [at]", "添加代码调用引用",
	"axd", " addr [at]", "添加数据引用",
	"axF", " [flg-glob]", "查找标志的数据/代码引用",
	"axf", "[?] [addr]", "查找从此地址的数据/代码引用",
	"axff", "[*jqQ] [addr]", "查找此函数的数据/代码引用",
	"axg", "[*j] [addr]", "显示到达当前函数的交叉引用图",
	// "axg*", " [addr]", "显示到达给定地址的交叉引用图，使用 .axg*;aggv",
	// "axgj", " [addr]", "以 json 格式显示到达当前函数的交叉引用图",
	"axi", " addr [at]", "添加间接代码引用（参见 ax?）",
	"axj", "", "添加跳转引用", // 以 json 格式列出引用", // R2_600 XXX 这是错误的。axj 必须用 json 列出交叉引用
	"axl", "[jcq]", "列出交叉引用（axlc = 计数, axlq = 简洁, axlj = json）",
	"axm", " addr [at]", "复制指向 addr 的数据/代码引用也指向 curseek（或 at）",
	"axq", "", "以简洁/人类可读格式列出引用",
	"axr", " addr [at]", "添加数据读取引用",
	"axs", " addr [at]", "添加字符串引用",
	"axt", "[?] [addr]", "查找到此地址的数据/代码引用",
	"axv", "[?] [addr]", "列出局部变量读写执行引用",
	"axw", " addr [at]", "添加数据写入引用",
	NULL
};
...
static RCoreHelpMessage help_msg_axl = {
	"用法:", "axl[jcq]", "显示全局交叉引用",
	"axl", "", "列出所有交叉引用",
	"axlj", "", "以 json 格式列出交叉引用",
	"axlc", "", "统计注册的交叉引用数量",
	"axlq", "", "以简洁模式列出交叉引用（axq）",
	NULL
};

static RCoreHelpMessage help_msg_axv = {
	"用法:", "axv[?j]", "显示当前函数中局部变量的交叉引用",
	"axv", " ([addr])", "可选地可以指定地址而不是当前搜索",
	"axvj", " ([addr])", "以 json 显示",
	NULL
};

static RCoreHelpMessage help_msg_axt = {
	"用法:", "axt[?gq*]", "查找到此地址的数据/代码引用",
	"axtj", " ([addr])", "查找到此地址的数据/代码引用并以 json 格式打印",
	"axtg", " ([addr])", "显示根据交叉引用生成图形的命令",
	"axtq", " ([addr])", "查找并以简洁模式列出数据/代码引用",
	"axtm", " ([addr])", "以 'make' 语法显示交叉引用（参见 aflm 和 axfm）",
	"axt*", " ([addr])", "同 axt，但输出为 r2 命令",
	NULL
};

static RCoreHelpMessage help_msg_axf = {
	"用法:", "axf[?gq*]", "查找从此地址的数据/代码引用",
	"axfj", " [addr]", "查找到此地址的数据/代码引用并以 json 格式打印",
	"axfg", " [addr]", "显示根据交叉引用生成图形的命令",
	"axfq", " [addr]", "查找并以简洁模式列出数据/代码引用",
	"axfm", " [addr]", "以 'make' 语法显示引用（参见 aflm 和 axtm）",
	"axf*", " [addr]", "同 axf，但输出为 r2 命令",
	NULL
};
