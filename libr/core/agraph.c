...
static RCoreHelpMessage help_msg_visual_graph = {
	":e cmd.gprompt=agft", "在一侧显示微型图",
	"@",            "在 0 和 1 之间切换 graph.layout",
	"[]",           "调整 graph.bb.maxwidth",
	"+/-/0",        "放大/缩小/默认缩放",
	";",            "在当前基本块中添加注释",
	". (点号)",      "将图居中到当前节点",
	", (逗号)",      "切换 graph.few",
	...
	"|",            "设置 cmd.gprompt",
	"_",            "进入 HUD 选择器",
	">",            "显示函数调用图 (参见 graph.refs)",
	"<",            "显示程序调用图 (参见 graph.refs)",
	"(",            "反转基本块中最后一条指令的条件分支",
	")",            "轮换 asm.emu 和 emu.str",
	"%",            "在反汇编中查找 (pdr~sentence) 并在图中导航到它",
	"Home/End",     "转到画布顶部/底部",
	"Page-UP/DOWN", "向上/向下滚动画布",
	"b",            "可视化浏览内容",
	"c",            "切换图形光标模式",
	"C",            "切换 scr.colors",
	"d",            "重命名函数",
	"D",            "切换混合图形+反汇编模式",
	"e",            "轮换 graph.edges (显示/隐藏边)",
	"E",            "轮换 graph.linemode (直角/对角线)",
	"F",            "进入标志选择器",
	"g",            "转到/寻址到给定偏移",
	"G",            "调试跟踪调用图 (使用 dtc 生成)",
	"hjkl/HJKL",    "根据图形光标滚动画布或节点 (大写表示更快)",
	"i/I",          "按索引选择输入/输出节点",
	"mK/'K",        "标记/转到给定键 (与 vim 或 r2 可视化中相同)",
	"M",            "更改鼠标模式 (垂直/水平滚动)",
	"n/N",          "下一个/上一个 scr.nkey (函数/标志等)",
	"o([A-Za-z]*)", "跟随由快捷键标识的 jmp/call (如 ;[oa])",
	"O",            "切换 asm.pseudo 和 asm.esil",
	"p/P",          "轮换图形模式 (普通、显示偏移、微型图、摘要)",
	"q",            "返回可视化模式",
	"r",            "切换 jmphints/leahints",
	"R",            "随机化颜色",
	"s/S",          "单步步入 / 单步步过",
	"tab",          "选择下一个节点",
	"TAB",          "选择上一个节点",
