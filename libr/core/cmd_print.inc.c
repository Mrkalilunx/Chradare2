...
static RCoreHelpMessage help_msg_pa = {
	"用法: pa[edD]", "[asm|hex]", "打印（反）汇编",
	"pa", " [汇编]", "打印给定汇编表达式的十六进制对",
	"paD", " [十六进制对]", "从十六进制对打印汇编表达式并显示十六进制对",
	"pad", " [十六进制对]", "从十六进制对打印汇编表达式（pdx, pix 的别名）",
	"pade", " [十六进制对]", "从十六进制对打印 ESIL 表达式",
	"pae", " [汇编]", "打印给定汇编表达式的 ESIL 表达式",
	NULL
};

static RCoreHelpMessage help_msg_psz = {
	"用法: psz[jl]", "", "打印以零结尾的字符串",
	"psz", "", "打印以零结尾的字符串",
	"psz*", "", "r2 命令将以零结尾的字符串写入此处",
	"pszj", "", "以 JSON 格式打印以零结尾的字符串",
	"pszl", "", "打印当前地址的以零结尾的字符串的长度",
	NULL
};

static RCoreHelpMessage help_msg_psp = {
	"用法: psp[124j]", "", "打印 Pascal 字符串",
	"psp", "", "打印以零结尾的字符串，长度由第一个字节定义（psp1）",
	"psp1", "", "与 psp 相同",
	"psp2", "", "与 psp 相同，但长度为 2 字节",
	"psp4", "", "与 psp 相同，但使用 4 字节双字（遵循 cfg.bigendian）作为长度",
	"pspj", "", "以 JSON 格式打印 Pascal 字符串",
	"pspw", "", "打印以零结尾的 16 位宽字符串，长度由第一个字节定义（pspw1）",
	"pspw1", "", "与 pspw 相同",
	"pspw2", "", "与 pspw 相同，但长度为 2 字节",
	"pspw4", "", "与 pspW 相同，但使用 4 字节双字（遵循 cfg.bigendian）作为长度",
	"pspwj", "", "以 JSON 格式打印 Pascal 16 位宽字符串",
	"pspW", "", "打印以零结尾的 32 位宽字符串，长度由第一个字节定义（pspW1）",
	"pspW1", "", "与 pspW 相同",
	"pspW2", "", "与 pspW 相同，但长度为 2 字节",
	"pspW4", "", "与 pspW 相同，但使用 4 字节双字（遵循 cfg.bigendian）作为长度",
	"pspWj", "", "以 JSON 格式打印 Pascal 32 位宽字符串",
	NULL
};

static RCoreHelpMessage help_msg_p8 = {
	"用法: p8[*fjx]", " [len]", "8 位字节的十六进制对列表（见 pcj）",
	"p8", " ([len])", "打印十六进制对字符串",
	"p8,", "", "以逗号分隔的 0x十六进制字节",
	"p8*", "", "显示 r2 命令以写入此块",
	"p8b", "", "打印基本块的十六进制对",
	"p8d", "", "以十进制表示的字节值的空格分隔列表",
	"p8f", "[j]", "打印函数的十六进制对（线性）",
	"p8fm", "[j]", "打印线性函数字节:掩码模式（零填充的 bbgaps）",
	"p8j", "", "以 JSON 数组格式打印十六进制对",
	"p8s", "", "以空格分隔的十六进制字节",
	"p8x", "", "打印遵循 hex.cols 的十六进制对",
	NULL
};

static RCoreHelpMessage help_msg_pm = {
	"用法: pm", "[文件|目录]", "设置 libmagic 参考文件或目录（见 /m?）",
	"pm", " [文件|目录]", "设置 libmagic 参考（见 /m?）",
	"e", " dir.magic", "默认为 " R_JOIN_2_PATHS(R2_PREFIX, R2_SDB_MAGIC),
	NULL
};
...
static RCoreHelpMessage help_msg_pp = {
	"用法: pp[d]", "", "打印模式",
	"pp0", "", "打印填充零的缓冲区",
	"pp1", "", "打印增量字节模式（遵循当前地址和 bsize 的低位）",
	"pp2", "", "打印增量字模式",
	"pp4", "", "打印增量双字模式",
	"pp8", "", "打印增量四字模式",
	"ppa", "[lu]", "拉丁字母（小写，大写限制）",
	"ppd", "", "打印 debruijn 模式（见 ragg2 -P, -q 和 wopD）",
	"ppf", "", "打印填充 0xff 的缓冲区",
	"ppn", "", "数字引脚模式",
	NULL
};
...
static RCoreHelpMessage help_msg_p6 = {
	"用法: p6[d|e][s|z]", " [len]", "base64 解码/编码",
	"p6d", "[s|z] [len]", "将当前块解码为 base64",
	"p6e", "[s|z][len]", "将当前块编码为 base64",
	"p6ez", "", "编码为 base64 的零结尾字符串",
	"p6es", " hello world", "将给定字符串编码为 base64",
	"p6ds", " AAA=", "解码给定的 base64 字符串",
	"p6dz", "", "解码块中的以零结尾的 base64 字符串",
	NULL
};

static RCoreHelpMessage help_msg_pF = {
	"用法: pF[apdbA][*vqj]", "[len]", "解析 ASN1, PKCS, X509, DER, protobuf, axml",
	"pFa", "[jqt] [len]", "从当前块解码 ASN1/DER（PEM 是 B64(DER)）",
	"pFA", "[j] [len]", "从当前块解码 Android 二进制 XML",
	"pFb", "[vj] [len]", "以（详细，JSON）格式解码原始 proto 缓冲区",
	"pFB", "[j] [len]", "从当前块解码 iOS 二进制 PLIST",
	"pFo", "[j] [len]", "解码 ASN1 OID",
	"pFp", "[j] [len]", "解码 PKCS7",
	"pFx", "[j] [len]", "与 X509 相同",
	"pFX", " [len]", "打印解压缩的 xz 块",
	NULL
...
static RCoreHelpMessage help_msg_pri = {
	...
	"pris", "", "sixel 图像",
	"prig", "", "灰度图像",
	"prir", "", "RGB 图像（与 pri 相同）",
	"pri4", "", "RGBA 图像",
	NULL
};
static RCoreHelpMessage help_msg_pr = {
	"用法: pr[glx]", "[size]", "打印 N 个原始字节",
	"prc", "[=fep..]", "以调色板中的颜色打印字节",
	"prg", "[?]", "打印原始 GUNZIP 过的块",
	"pri", "[aA12r]", "打印原始图像，1 位图像，遵循 hex.cols",
	"print", "[f][ln]", "打印，println，printf，printfln",
	"prl", "", "打印带有行偏移的原始数据",
	"prx", "", "打印具有真实偏移的可打印字符（hyew）",
	"prz", "", "打印原始零结尾字符串",
	NULL
};

static RCoreHelpMessage help_msg_prg = {
	"用法: prg[?ilor]", " [len]", "打印原始解压缩块",
	"prg", "", "打印当前块的gunzip解压数据",
	"prgl", "", "使用LZ4解压缩当前块（调整块大小）",
	"prgi", "", "显示解压缩时消耗的字节数",
	"prgo", "", "显示解压缩后的输出字节数",
	"prgr", "", "打印原始deflate解压数据（无ZLIB头）",
	NULL
};

static RCoreHelpMessage help_msg_amper = {
	"用法:", "&[-|<cmd>]", "管理任务（警告：实验性。请谨慎使用！）",
	"&", "", "列出所有任务（'jobs' 命令的别名）",
	"&", " <cmd>", "在新的后台任务中运行 <cmd>（'bg' 的别名）",
	"&:", "<cmd>", "将 <cmd> 排队以便在可能时执行",
	"&t", " <cmd>", "在新的瞬态后台任务中运行 <cmd>（完成后自动删除）",
	"&j", "", "列出所有任务（以 JSON 格式）",
	"&=", " 3", "显示任务 3 的输出",
	"&b", " 3", "中断任务 3",
	"&w", "", "等待排队的命令并执行它们（^C 结束）",
	...
	"&", "-*", "删除所有已完成的任务",
	"&?", "", "显示此帮助",
	"&&", " 3", "等待直到任务 3 完成（'fg' 的别名）",
	"&&", "", "等待直到所有任务完成（与 'fg' 相同）",
	NULL
};

static RCoreHelpMessage help_msg_p = {
	"用法:", "p[=68abcdDfiImrstuxz] [arg|len] [@addr]", "",
	// "p", "[b|B|xb] [len] ([S])", "绑定 N 位跳过 S 字节",
	// "p", "[iI][df] [len]", "打印 N ops/bytes (f=func) (见 pi? 和 pdi)",
	//"p", "[kK] [len]", "在随机艺术中打印密钥（K 是马赛克）",
	"p-", "[?][jh] [mode]", "条形|json|直方图块（模式: e?search.in）",
	"p2", " [len]", "8x8 2bpp-tiles",
	"p3", " [file]", "打印当前块的 3D 立体图像",
	"p6", "[de] [len]", "base64 解码/编码",
	"p8", "[?][bdfsjx] [len]", "8 位字节的十六进制对列表",
	"p=", "[?][bep] [N] [L] [b]", "显示熵/可打印字符/字符条",
	"pa", "[?][edD] [arg]", "pa:assemble pa[dD]:disasm 或 pae: 从十六进制生成 esil",
	"pA", "[n_ops]", "显示 n_ops 地址和类型",
	"pb", "[?] [n]", "N位的比特流",
	"pB", "[?] [n]", "N字节的比特流",
	"pc", "[?][p] [len]", "输出C（或python）格式",
	"pC", "[aAcdDxw] [rows]", "以列的形式打印反汇编（参见hex.cols和pdi）",
	"pd", "[?] [sz] [a] [b]", "反汇编N个操作码（pd）或N个字节（pD）",
	"pf", "[?][.name] [fmt]", "打印格式化数据（pf.name，pf.name $<expr>）",
	"pF", "[?][apx]", "打印asn1、pkcs7或x509",
	"pg", "[?][x y w h] [cmd]", "创建新的可视化小工具或打印它（参见pg?以获取详细信息）",
	"ph", "[?][=|hash] ([len])", "计算块的哈希值",
	"pi", "[?][bdefrj] [num]", "打印指令",
	"pI", "[?][iI][df] [len]", "打印N条指令/字节（f=func）",
	"pj", "[?] [len]", "以缩进的JSON格式打印",
	"pk", " [len]", "以随机艺术打印密钥",
	"pK", " [len]", "以随机艺术全屏马赛克打印密钥",
	"pl", "[?][format] [arg]", "打印数据列表（pl Ffvc）",
	"pm", "[?] [magic]", "打印libmagic数据（参见pm?和/m?）",
	"po", "[?] hex", "打印应用于块的操作（参见po?）",
	"pp", "[?][sz] [len]", "打印模式，参见pp?以获取更多帮助",
	"pq", "[?][is] [len]", "打印前N个字节的二维码",
	"pr", "[?][glx] [len]", "打印N个原始字节（按行或十六进制块，'g'unzip）",
	"ps", "[?][pwz] [len]", "打印pascal/宽/零终止字符串",
	"pt", "[?][dn] [len]", "打印不同的时间戳",
	"pu", "[w] [len]", "打印N个URL编码字节（w=宽）",
	"pv", "[?][ejh] [mode]", "显示给定大小的值（1、2、4、8）",
	"pwd", "", "显示当前工作目录",
	"px", "[?][owq] [len]", "N字节的十六进制转储（o=八进制，w=32位，q=64位）",
	"py", "([-:file]) [expr]", "打印剪贴板（yp）运行python脚本（py:file）单行py print(1)或stdin吸入py-",
	"pz", "[?] [len]", "打印缩放视图（参见pz?以获取帮助）",
	"pkill", " [process-name]", "杀死所有具有给定名称的进程",
	"pushd", " [dir]", "切换到目录并将当前目录推送到堆栈",
	"popd", "[-a][-h]", "从堆栈顶部弹出目录并切换到它",
	NULL
};

static RCoreHelpMessage help_msg_pxd = {
	"用法:", "pxd[1248] ([len])", "显示十进制字节/短整型/字/双字转储",
	"pxd", "", "显示基于10的有符号十六进制转储",
	"pxd1", "", "显示字节十六进制转储（int8_t）",
	"pxd2", "", "显示短整型十六进制转储（int16_t）",
	"pxd4", "", "显示双字十六进制转储（int32_t）",
	"pxd8", "", "显示四字十六进制转储（int64_t）",
	NULL
};

static RCoreHelpMessage help_msg_pxu = {
	"用法:", "pxu[1248] ([len])", "显示无符号十进制字节/短整型/字/双字转储",
	"pxu", "", "显示基于10的无符号十六进制转储",
	"pxu1", "", "显示字节十六进制转储（int8_t）",
	"pxu2", "", "显示短整型十六进制转储（int16_t）",
	"pxu4", "", "显示双字十六进制转储（int32_t）",
	"pxu8", "", "显示四字十六进制转储（int64_t）",
	NULL
};

static RCoreHelpMessage help_msg_prc = {
	"用法:", "prc=[#] ([size]) ", "参见p=?以查找#的有效值",
	"prc", "", "以调色板中的颜色打印字节",
	"prcb", "", "以调色板中的颜色打印半字节位",
	"prcn", "", "以调色板中的颜色打印半字节",
	"prc=e", "", "熵",
...
static RCoreHelpMessage help_msg_p_equal = {
	"用法:", "p=[=bep?][qj] [N] ([len]) ([offset]) ", "显示熵/可打印字符/字符条",
	"e ", "zoom.in", "指定缩放范围",
	"p=", "", "以条形图打印当前块的字节",
	"p==", "[..]", "与p=相同的子命令，使用列条而不是行",
	"p=", "0", "每个文件大小/块大小的0x00字节数量",
	"p=", "2", "短条（有符号int16），适合波形",
	"p=", "a", "分析bbs地图",
	"p=", "A", "分析统计地图（参见p-）",
	"p=", "b", "与上述相同",
	"p=", "c", "每个块的调用次数",
	"p=", "d", "块中的唯一字节的最小/最大/数量",
	"p=", "e", "每个文件大小/块大小的熵",
	"p=", "F", "每个文件大小/块大小的0xFF字节数量",
	"p=", "i", "每个块的无效指令数量",
	"p=", "j", "块中的跳转和条件跳转数量",
	"p=", "m", "块中的标志和标记数量",
	"p=", "p", "每个文件大小/块大小的可打印字节数量",
	"p=", "s", "系统调用和特权指令的数量",
	"p=", "z", "块中字符串的字符数量",
	NULL
};
...
static RCoreHelpMessage help_msg_pj = {
	"用法:", "pj[..] [size]", "",
	"pj", "", "以缩进的JSON格式打印当前块",
	"pj.", "", "从0到当前偏移量以缩进的JSON格式打印",
	"pj..", "", "从0到当前偏移量打印JSON路径",
	NULL
};

static RCoreHelpMessage help_msg_p_minus = {
	"用法:", "p-[hej] [nblocks] ", "条形图|json|直方图块",
	"p-", " [nblocks]", "显示文件边界中的元数据的ASCII艺术条",
	"p-e", " [nblocks]", "显示每个块的熵的ASCII艺术条",
	"p-h", " [nblocks]", "显示每个块的元数据的直方图分析",
	"p-j", " [nblocks]", "显示json格式",
	NULL
};

static RCoreHelpMessage help_msg_pdf = {
	"用法:","pdf[sj]"," # 反汇编函数（需要用'af'或类似的分析）",
	"pdf","","以线性方式反汇编函数（参见pdfr）",
	"pdfj","","以json格式反汇编函数（参见pdfJ）",
	"pdfr","","递归反汇编函数（pdr的别名）",
	"pdfs","","反汇编函数摘要（参见pdsf）",
	NULL,
};
...
static RCoreHelpMessage help_msg_pd = {
	"用法:", "p[dD][ajbrfils] [[-]len]", " # 打印N字节/指令bw/前进",
	"注意: ", "len", "参数可以是负数",
	"注意: ", "", "在空命令上按ENTER将重复上一个打印命令到下一页",
	"pD", " N", "反汇编N字节",
	"pd", " -N", "向后反汇编N条指令",
	"pd", " N", "反汇编N条指令",
	"pd--", " N", "N条指令的上下文反汇编",
	"pda", "", "逐字节反汇编所有可能的操作码",
	"pdaj", "", "以JSON格式逐字节反汇编所有可能的操作码",
	"pdb", "[j]", "反汇编基本块（j表示JSON）",
	"pdc", "[?][c]", "伪反汇编器输出为C类似语法",
	"pdC", "", "显示N条指令中找到的注释",
	"pde", "[q|qq|j] N", "根据当前PC的执行流反汇编N条指令",
	"pdo", " N", "将N条指令的esil表达式转换为C（pdO表示字节）",
	"pdf", "[?]", "反汇编函数",
	"pdi", "", "类似'pi'，带偏移量和字节",
	"pdj", "", "以json格式反汇编",
	"pdJ", "", "以json格式格式化反汇编",
	"pdk", "[?]", "反汇编类的所有方法",
	"pdl", "", "显示指令大小",
	"pdp", "", "通过跟随指针反汇编以读取rop链",
	"pdr", "", "在函数图中递归反汇编",
	"pdr.", "", "从当前基本块在函数图中递归反汇编",
	"pdR", "", "在不分析函数的情况下递归反汇编块大小字节",
	"pds", "[?]", "打印反汇编摘要，显示引用的名称",
	"pdu", "[aceios?]", "反汇编指令直到条件",
	"pd,", " [n] [query]", "在表中反汇编N条指令（参见dtd以获取调试跟踪）",
	"pdx", " [hex]", "pad或pix的别名",
	NULL
};

static RCoreHelpMessage help_msg_pds = {
	"用法:", "pds[*|b|f|s] [N]", "打印反汇编中的字符串",
	"pds", "[?]", "打印反汇编摘要，显示引用的名称",
	"pds*", "[?]", "打印r2命令，为每个字符串引用添加注释",
	"pdsb", " [N]", "基本块摘要",
	"pdsf", "[sjq]", "显示字符串、调用、变量、引用等的函数摘要",
	"pdss", " [N]", "当前函数中的字符串摘要",
	NULL
};

static RCoreHelpMessage help_msg_pde = {
	"用法:", "pde[q|qq|j] [N]", "根据当前PC的执行流反汇编N条指令",
	"pde", "", "根据当前PC的执行流反汇编N条指令",
	"pdeq", "", "根据当前PC的执行流反汇编N条指令（类似pdi）",
	"pdeqq", "", "根据当前PC的执行流反汇编N条指令（类似pi）",
	"pdej", "", "以JSON格式根据当前PC的执行流反汇编N条指令",
	NULL
};
...
static RCoreHelpMessage help_msg_ph = {
	"用法:", "ph", " [算法] ([大小])",
	"ph", " md5", "计算当前块的md5哈希",
	"ph", ":md5", "与'ph md5'相同（冒号作为空格）",
	"ph", " sha1 32 @ 0x1000", "计算从0x1000开始的32字节的sha1",
	"ph", "", "列出可用的哈希插件（与Lh和phl/phL相同）",
	"phj", "", "以json格式列出可用的哈希插件",
	NULL
};

static RCoreHelpMessage help_msg_pdu = {
	"用法:", "pdu[acios][j]", "反汇编指令直到条件",
	"pdua", "[j] [addr]", "反汇编直到地址",
	"pduc", "[j]", "反汇编直到调用",
	//"pdue", "[j] [expr]", "反汇编直到esil表达式",
	"pdui", "[j] [inst]", "反汇编直到指令（例如：add esp, 0x20）",
	"pduo", "[j] [opcode]", "反汇编直到操作码（例如：mul）",
	"pdus", "[j]", "反汇编直到系统调用",
	NULL
};
...
static RCoreHelpMessage help_msg_pfb = {
	"用法:", "pfb", "打印格式化的比特域",
	"pfb", " [fmt] [fnames]", "以ASCII艺术格式打印格式化的比特域",
	"pfbc", " [fmt] [fnames]", "与pfb相同，但使用C语法",
	"pfbj", " [fmt] [fnames]", "与pfb相同，但以json输出",
	"pfbq", " [fmt] [fnames]", "与pfb相同，但更安静的单行",
	"pfbd", " [fmt] [fnames]", "与pfb相同，但出于调试原因",
	"示例:", "", "",
	" ", "pfb 3b4b foo bar", "2个比特域，第一个为3位，第二个为4位",
	" ", "pfb 3b+4b foo bar", "与上述相同，+号被忽略",
	" ", "pfb 3b..4b foo bar", "与上述相同，但由2个未使用的位分隔",
	" ", "pfb 3b2.4b foo bar", "与上述相同，可以使用数字和点",
	NULL
};
...
static RCoreHelpMessage help_msg_pf = {
	"用法:", PF_USAGE_STR, "",
	"命令:", "", "",
	"pf", " fmt", "使用给定的格式字符串显示数据。请参见 'pf\?\?' 和 'pf\?\?\?'.",
	"pf", "?", "命令帮助",
	"pf", "??", "格式字符帮助",
	"pf", "???", "显示用法示例",
	"pf* ", "fmt_name|fmt", "使用（命名）格式作为 r2 标志创建命令显示数据",
	"pf.", "", "列出所有格式定义",
	"pf.", "fmt_name", "使用命名格式显示数据",
	"pf.", "fmt_name.field_name", "使用命名格式显示特定数据字段",
	"pf.", "fmt_name.field_name=33", "为命名格式中的指定字段设置新值",
	"pf.", "fmt_name.field_name[i]", "显示数组字段名称的元素 i",
	"pf.", "fmt_name [0|cnt]fmt", "定义一个新的命名格式",
	"pf?", "fmt_name", "显示命名格式的定义",
	"pfb", "[?] bitfmt", "打印格式化的位域",
	"pfc", " fmt_name|fmt", "作为 C 字符串使用（命名）格式显示数据",
	"pfd.", "fmt_name", "作为 graphviz 命令使用命名格式显示数据",
	"pfj ", "fmt_name|fmt", "以 JSON 格式使用（命名）格式显示数据",
	"pfo", " fdf_name", "加载格式定义文件（fdf）",
	"pfo", "", "列出所有格式定义文件（fdf）",
	"pfq", " fmt ...", "安静打印格式（不显示地址）",
	"pfs", "[.fmt_name|fmt]", "打印（命名）格式的字节大小",
	"pfv.", "fmt_name[.field]", "仅打印命名格式的值。适用于单行命令",
	NULL
};
...
static RCoreHelpMessage help_detail_pf = {
	...
	" ", "c", "字符（有符号字节）",
	" ", "C", "十进制字节",
	" ", "d", "双字（4 字节十六进制）（见 'i' 和 'x'）",
	" ", "D", "反汇编一个操作码",
	" ", "e", "暂时交换字节序",
	" ", "E", "解析枚举名称（见 t?）",
	" ", "f", "浮点值（4 字节）",
	" ", "F", "双精度值（8 字节）",
	" ", "G", "长双精度值（16 字节（10 字节填充））",
	" ", "i", "有符号整数值（4 字节）（见 'd' 和 'x'）",
	" ", "n", "下一个字符指定有符号值的大小（1、2、4 或 8 字节）",
	" ", "N", "下一个字符指定无符号值的大小（1、2、4 或 8 字节）",
	" ", "o", "八进制值（4 字节）",
	" ", "p", "指针引用（2、4 或 8 字节）",
	" ", "q", "四字（8 字节）",
	" ", "Q", "uint128_t（16 字节）",
	" ", "r", "CPU 寄存器 pf r (eax)plop",
	" ", "s", "32 位指向字符串的指针（4 字节）",
	" ", "S", "64 位指向字符串的指针（8 字节）",
	" ", "t", "UNIX 时间戳（4 字节）",
	" ", "T", "显示缓冲区的前十个字节",
	" ", "u", "uleb128（可变长度）",
	" ", "w", "字（2 字节无符号短整型十六进制）",
	" ", "x", "0xHEX 值和标志（fd @ addr）（见 'd' 和 'i'）",
	" ", "X", "显示格式化的十六进制对",
	" ", "z", "以 null 结尾的字符串",
	" ", "Z", "以 null 结尾的宽字符串",
	" ", "?", "数据结构 pf ? (struct_name)example_name",
	" ", "*", "下一个字符是指针（遵循 asm.bits）",
	" ", "+", "切换显示每个偏移量的标志",
	" ", ":", "跳过 4 字节",
	" ", ".", "跳过 1 字节",
	" ", ";", "回退 4 字节",
...
static RCoreHelpMessage help_detail2_pf = {
	"用法:", PF_USAGE_STR, "",
	"示例:", "", "",
	"pf", " 3xi foo bar", "3-数组结构，每个都有命名字段：'foo' 作为十六进制，'bar' 作为整数",
	"pf", " B (BitFldType)arg_name`", "位域类型",
	"pf", " E (EnumType)arg_name`", "枚举类型",
	"pf", " *z*i*w nb name blob", "打印带有给定标签的指针",
	"pf", " iwq foo bar troll", "打印 iwq 格式，foo、bar、troll 作为字段的相应名称",
	"pf", " 0iwq foo bar troll", "与上面相同，但视为联合（所有字段在偏移量 0）",
	"pf.", "obj xxdz prev next size name", "将 obj 格式定义为 xxdz",
	"pf", " obj=xxdz prev next size name", "与上面相同",
	"pf.", "plop ? (troll)mystruct", "使用之前定义的结构 troll",
	"pfj.", "plop @ 0x14", "在给定偏移量应用格式对象",
	"pf", " 10xiz pointer length string", "打印大小为 10 的 xiz 结构数组及其字段名称",
	"pf", " 5sqw string quad word", "打印带有 sqw 结构及其字段名称的数组",
	"pf", " 3? (bifc)", "打印 3 次以下格式 (bifc)",
	"pf", " [4]w[7]i", "打印 4 个字的数组，然后是 7 个整数的数组",
	"pf", " ic...?i foo bar \"(pf xw yo foo)troll\" yo", "打印嵌套的匿名结构",
	"pf", " :..x", "打印当前偏移量 6 字节处的值",
	"pf", " \";x\"", "打印当前偏移量前 4 字节处的值",
	"pf", " [10]z[3]i[10]Zb", "打印固定大小的字符串、宽字符和变量",
	"pfj", " +F @ 0x14", "打印给定偏移量的内容及标志",
	"pf", " n2", "打印有符号短整型（2 字节）值。使用 N 而不是 n 打印无符号值",
	"pf", " [2]? (plop)structname @ 0", "打印结构数组",
	"pf", " eqew bigWord beef", "交换字节序并打印带有给定标签",
	"pf", ".foo rr (eax)reg1 (eip)reg2", "创建引用寄存器值的对象",
	"pf", " tt troll plop", "打印带有标签 troll 和 plop 的时间戳",
	NULL
};
...
static RCoreHelpMessage help_msg_pi = {
	...
	"pid", "", "pdi 的别名",
	"pie", "[?]", "打印偏移量 + esil 表达式",
	"piE", "[?]", "与 pie 相同，但使用字节数量而不是指令",
	"pif", "[?]", "打印函数的指令",
	"pij", "", "以 JSON 格式打印 N 条指令",
	"pir", "", "像 'pdr' 但输出为 'pI'",
	"piu", "[q] [optype]", "反汇编直到找到给定 optype 的指令（见 /atl）",
	"pix", " [hexpairs]", "pdx 和 pad 的别名",
	NULL
};

static RCoreHelpMessage help_msg_piE = {
	"用法:", "piE[q]", " # 打印 N 字节的 esil",
	"piE", "", "打印 N 字节中找到的指令的 esil",
	"piEq", "", "与上面相同，但不显示指令地址",
	// "piEj", "", "相同但以 JSON 格式",
	NULL
};
...
static RCoreHelpMessage help_msg_pie = {
	"用法:", "pie[bqf]", " # 打印 N 条指令的 esil",
	"pie", "", "打印 N 条指令的 esil",
	"pieb", "", "pie $Fi 的别名",
	"pieq", "", "与上面相同，但隐藏偏移量",
	"pief", "", "打印所有函数基本块的 esil",
	"piefq", "", "与上面相同，但隐藏偏移量",
	// "piej", "", "相同但以 JSON 格式",
	NULL
};
...
static RCoreHelpMessage help_msg_pif = {
	"用法:", "pif[cj]", " # 打印函数中的指令",
	"pif", "", "打印函数指令",
	"pifj", "", "与上面相同，但以 JSON 格式",
	"pifc", "", "打印此函数的所有调用", // pif~call?
	"pifcj", "", "以 JSON 格式打印此函数的所有调用",
	NULL
};

static RCoreHelpMessage help_msg_po = {
	"用法:", "po[24aAdlmorsxS]", " [hexpairs] @ addr[!bsize] (另见 poke)",
	"po[24aAdlmorsx]", "", "没有十六进制对值时，使用剪贴板",
	"po2", " [val]", "2= 2 字节字节序交换",
	"po4", " [val]", "4= 4 字节字节序交换",
	"poa", " [val]", "+= 加法（例如：poa 0102）",
	"poA", " [val]", "&= 与",
	"pod", " [val]", "/= 除法",
	"poD", " [algo] [key] [iv]", "打印块解密",
	"poE", " [algo] [key] [iv]", "打印块加密",
	"pol", " [val]", "<<= 左移",
	"pom", " [val]", "*= 乘法",
	"poo", " [val]", "|= 或",
	"por", " [val]", ">>= 右移",
	"pos", " [val]", "-= 减法",
	"pox", " [val]", "^= 异或（例如：pox 0x90）",
	"poS", " [algo] [key]", "计算并打印块签名",
	NULL
};

static RCoreHelpMessage help_msg_pq = {
	"用法:", "pq[?z] [len]", "生成 ASCII 艺术的 QR 码",
	"pq", " 32", "打印当前 32 字节的 QR 码",
	"pqz", "", "打印当前偏移量的当前字符串的 QR 码",
	NULL
};

static RCoreHelpMessage help_msg_ps = {
	"用法:", "ps[abijqpsuwWxz+] [len]", "打印字符串（可选指定长度）",
	"ps", "[j]", "打印直到第一个不可打印字符的字符串",
	"ps+", "[j]", "打印 libc++ std::string（相同字节序、ASCII、以零结尾）",
	"psa", "", "打印任何类型的字符串（psp/psw/psW/psz/..）",
	"psb", "", "打印当前块中的字符串",
	"psi", "", "打印当前查找位置中的字符串",
	"psn", "[l] [len]", "打印直到换行符的字符串",
	"psp", "[?][j]", "打印 Pascal 字符串",
	"psq", "", "pqs 的别名",
	"pss", "", "打印屏幕中的字符串（换行宽度）",
	"pso", "[j]", "打印由立即数构造的字符串（Swift）",
	"psu", "[zj]", "打印 utf16 Unicode（JSON）",
	"psw", "[j]", "打印 16 位宽字符串",
	"psW", "[j]", "打印 32 位宽字符串",
	"psx", "", "显示带有转义字符的字符串",
	"psz", "[?] [lj]", "打印以零结尾的字符串",
...
static RCoreHelpMessage help_msg_pt = {
	"用法: pt", "[dn]", "打印时间戳",
	"pt.", "", "打印当前时间",
	"pt", "", "打印 UNIX 时间（32 位 cfg.bigendian）自 1970 年 1 月 1 日以来",
	"ptb", "", "打印 BEAT 时间（瑞士互联网时间）",
	"ptd", "", "打印 DOS 时间（32 位 cfg.bigendian）自 1980 年 1 月 1 日以来",
	"pth", "", "打印 HFS 时间（32 位 cfg.bigendian）自 1904 年 1 月 1 日以来",
	"ptn", "", "打印 NTFS 时间（64 位 cfg.bigendian）自 1601 年 1 月 1 日以来",
	NULL
};

static RCoreHelpMessage help_msg_pv = {
	"用法: pv[1248z][udj]", "", "打印给定大小和字节序的值（u 表示无符号，d 表示有符号十进制，j 表示 JSON）",
	"pv", "", "根据 asm.bits 打印字节",
	"pv1", "[udj]", "打印内存中的 1 个字节",
	"pv2", "[udj]", "打印内存中的 2 个字节",
	"pv4", "[udj]", "打印内存中的 4 个字节",
	"pv8", "[udj]", "打印内存中的 8 个字节",
	"pvp", "[udj]", "根据 asm.bits 打印 4 或 8 个字节",
	"pve", " [1234] ([bsize])", "以任意字节序打印值（使用 1234 顺序重新排序字节）",
	"pvz", "", "将值作为字符串打印（ps 的别名）",
	NULL
};

static RCoreHelpMessage help_msg_px = {
	"用法:", "px[0afoswqWqQ][f]", " # 打印十六进制",
	"px", "", "显示十六进制转储",
	"px--", "[n]", "上下文十六进制转储（pd--3 的十六进制转储版本）",
	"px/", "", "与 gdb 中的 x/ 相同（帮助 x）",
	"px*", "", "与 pc* 或 p8* 相同，像十六进制转储一样打印 r2 命令",
	"px0", "", "直到零字节的 8 位十六进制字节列表",
	"px3", "", "24 位十六进制转储",
	"pxa", "", "显示注释的十六进制转储",
	"pxA", "[?]", "显示操作分析颜色图",
	"pxb", "", "以十六进制转储形式转储位", // 应该是 px1?
	"pxB", "", "以位图形式转储位", // 应该是其他东西吗？:D
	"pxc", "", "显示带注释的十六进制转储",
	"pxd", "[?1248]", "有符号整数转储（1 字节，2 和 4）",
	"pxe", "", "表情符号十六进制转储！ :)",
	"pxf", "", "显示当前函数的十六进制转储",
	"pxh", "", "显示十六进制半字转储（16 位）",
	"pxH", "", "与上面相同，但每行一个",
	"pxi", "", "HexII 紧凑二进制表示",
	"pxl", "", "显示 N 行（行）的十六进制转储",
	"pxo", "", "显示八进制转储",
	"pxq", "", "显示十六进制四字转储（64 位）",
	"pxQ", "[q]", "与上面相同，但每行一个",
	"pxr", "[1248][qj]", "显示十六进制字引用（q=安静，j=json）",
	"pxs", "", "以稀疏模式显示十六进制",
	"pxt", "[*.] [origin]", "在 r2 命令中显示增量指针表",
	"pxu", "[?1248]", "无符号整数转储（1 字节，2 和 4）",
	"pxw", "", "显示十六进制字转储（32 位）",
	"pxW", "[q]", "与上面相同，但每行一个（q=安静）",
	"pxx", "", "显示 N 字节的无十六进制转储",
	"pxX", "", "显示 N 字的无十六进制转储",
	NULL
};
...
static RCoreHelpMessage help_msg_pz = {
	"用法: pz [len]", "", "打印缩放块（文件大小/N）",
	"e ", "zoom.maxsz", "块的最大大小",
	"e ", "zoom.from", "起始地址",
	"e ", "zoom.to", "结束地址",
	"e ", "zoom.byte", "指定如何计算每个字节",
	"pz0", "", "值为 '0' 的字节数",
	"pzF", "", "值为 0xFF 的字节数",
	"pze", "", "计算熵并扩展到 0-255 范围",
	"pzf", "", "块中的标志计数",
	"pzh", "", "头（第一个字节值）；这是默认模式",
	"pzp", "", "可打印字符的数量",
	"pzs", "", "范围内的字符串",
	// "警告：在大文件上，使用 'zoom.byte=h' 或限制范围\n");
	NULL
};

static RCoreHelpMessage help_msg_pxA = {
	"用法: pxA [len]", "", "显示操作分析颜色图",
	"$$", "", "int/swi/trap/new",
	"+-*/", "", "数学运算",
	"->", "", "推送",
	"..", "", "无操作",
...
static RCoreHelpMessage help_msg_pg = {
	"用法: pg[-]", "[asm|hex]", "打印（反）组装",
	"pg", " [x y w h cmd]", "添加新小工具",
	"pg", "", "打印所有小工具",
	"pg", "*", "将小工具打印为 r2 命令",
	"pg-", "*", "移除所有小工具",
	NULL
...
static int cmd_print(void *data, const char *input) {
	...
	case 's': // "ps"
		...
		case 'n': // "psn"
			if (input[1] == '?') {
				r_cons_printf (core->cons, "用法: psn[l] [len] - print string until newline or maxlen\n");
			} else {
				int len = core->blocksize;
				if (input[2] == ' ') {
