...
static RCoreHelpMessage help_msg_fR = {
	"用法: fR", " [from] [to] ([mask])", " # 重新定位与掩码匹配的标志，假设旧的和新的基地址",
	"fR", " entry0 dm~:1[1]", "重新基准入口点",
	NULL
};

static RCoreHelpMessage help_msg_fV = {
	"用法: fV", "[*-] [nkey] [offset]", " # 转储/恢复可视标记 (mK/'K)",
	"fV", " a 33", "将可视标记 'a' 设置为偏移量 33",
	"fV", "-", "删除所有可视标记",
	"fV", "*", "将可视标记转储为 r2 命令",
	"fV", "", "列出可视标记",
	NULL
...
static RCoreHelpMessage help_msg_f = {
	"用法: f", "[?] [flagname]", " # 管理偏移名称标志",
	"f", "", "列出标志（仅列出所选标志空间中的标志）",
	"f", " name 12 @ 33", "在偏移量 33 设置标志 'name'，长度为 12",
	"f", " name = 33", "别名为 'f name @ 33' 或 'f name 1 33'",
	"f", " name 12 33 [cmt]", "与上面相同 + 可选注释",
	"f?", "flagname", "检查标志是否存在，参见 ?? 和 ?!",
	"f.", " [*[*]]", "以 r2 命令列出每个函数的本地标志 (*)",
	"f.", "blah=$$+12", "设置名为 'blah' 的本地函数标签 (f.blah@$$+12)",
	"f.", "-blah", "删除名为 'blah' 的本地函数标签",
	"f.", " fname", "列出给定函数的所有本地标签",
	"f,", "", "标志的表格输出",
	"f*", "", "以 r 命令列出标志",
	"f-", ".blah@fcn.foo", "从当前查找位置的函数中删除本地标签 (也可以使用 f.-)",
	"f-", "name", "移除标志 'name'",
	"f-", "@addr", "移除地址表达式中的标志（与 f-$$ 或 f-0x.. 相同）",
	"f--", "", "删除所有标志和标志空间（去初始化）",
	"f+", "name 12 @ 33", "与上面相似，但如果不存在则创建新的",
	"f=", " [glob]", "列出带有标志偏移和大小的范围条形图",
	"fa", "[- ][name] [alias]", "为标志设置或取消设置别名表达式",
	"fb", " [addr]", "为新标志设置基地址",
	"fb", " [addr] [flag*]", "将与 'flag' 匹配的标志移动到相对地址",
	"fc", "[?][name] [color]", "为给定标志设置颜色",
	"fC", " [name] [cmt]", "为给定标志设置注释",
	"fd", "[?] addr", "返回标志+增量",
	"fD", "[?] rawname", "(去)混淆标志或设置新标志", // XXX 我们有 id 不是'
	"fe", " [name]", "创建标志名称.#num# 枚举标志。 (f.ex: fe foo @@= 1 2 3 4)",
	"fe-", "", "重置枚举计数器",
	"ff", " ([glob])", "到达下一个标志的字节距离（参见 sn/sp）",
	"fi", " [size] | [from] [to]", "显示当前块或范围中的标志",
	"fg", "", "将核心任务作业带到前台（参见 '&' 命令）",
	"fh", "[*] ([prefix])", "使用标志名称构建图形层次结构",
	"fj", "", "以 JSON 格式列出标志",
	"fq", "", "以安静模式列出标志",
	"fl", " (@[flag]) [size]", "显示或设置标志长度（大小）",
	"fla", " [glob]", "自动计算与 glob 匹配的所有标志的大小",
	"fm", " addr", "将当前偏移量的标志移动到新地址",
	"fn", "", "列出显示真实名称的标志（去混淆）",
	"fnj", "", "以 JSON 格式列出显示真实名称的标志（去混淆）",
	"fN", "", "显示当前地址的标志真实名称",
	"fN", " [[name]] [realname]", "设置标志真实名称（如果没有标志名称，则使用当前查找的名称）",
	"fo", "", "显示运气",
	"fO", " [glob]", "标志为序数 (sym.* func.* method.*)",
	//" fc [name] [cmt] ; 为特定标志设置执行命令"
	"fr", " [[old]] [new]", "重命名标志（如果没有新标志，则使用当前查找的标志）",
	"fR", "[?] [from] [to] [mask]", "重新定位所有与 from&~m 匹配的标志",
	"fs", "[?]+-*", "管理标志空间",
	"ft", "[?]*", "标志标签，有助于查找所有与某些词匹配的标志",
	"fV", "[*-] [nkey] [offset]", "转储/恢复可视标记 (mK/'K)",
	"fx", "[d]", "显示标志:标志大小的十六进制转储（或反汇编）",
	"fu", " [name]", "显示虚假标志信息（是的，这个名字真的很糟糕）",
	"fz", "[?][name]", "添加命名标志区域 -name 以删除。参见 fz?[name]",
	NULL
};

static RCoreHelpMessage help_msg_fc = {
	"用法: fc", "<flagname> [color]", " # 列出带有 'ecs' 的颜色",
	"fc", "", "与 fc 相同。",
	"fc", " color", "将颜色设置为当前偏移量的所有标志",
	"fc", " flag=color", "将颜色设置为给定标志。与 'fc color@flag' 相同",
	"fc.", "", "获取当前偏移量的所有标志的颜色",
	"fc-", "", "从当前偏移量中移除颜色",
	"fc-", "flagname", "从给定标志中移除颜色",
	"fc-*", "", "重置所有颜色标志",
	"fc*", "", "以 r2 命令列出所有标志颜色",
	"fc.*", "", "将颜色设置为当前偏移量的所有标志",
	NULL
...
static RCoreHelpMessage help_msg_feq = {
	"用法: f=", " [glob]", " # 使用 glob 表达式搜索标志名称",
	"f=", " str*", "过滤所有以 str 开头的标志",
	NULL
};

static RCoreHelpMessage help_msg_ft = {
	"用法: ft", "[?ln] ([k] [v ...])", "# 使用 glob 表达式搜索标志名称",
	"ft", " tag strcpy strlen ...", "为 'string' 标签设置单词",
	"ft", " tag", "获取所有匹配标志的偏移量",
	"ft", "", "列出所有标签",
	"ftn", " tag", "获取给定标签的匹配标志名称",
	"ftw", "", "在此文件中标记标志",
	"ftj", "", "以 JSON 格式列出所有标志标签",
	"ft*", "", "以 r2 命令列出所有标志标签",
	NULL
};

static RCoreHelpMessage help_msg_fD = {
	"用法: fD[*.j]", " [rawname]", " # 过滤/混淆原始符号名称以成为有效的标志名称",
	"fD", " rawname", "使用原始名称打印混淆的标志名称，参见 ' 命令前缀",
	"fD.", " rawname", "在当前偏移量中使用原始名称设置标志",
	"fDj", " rawname", "与 fD 相同，但输出为 json",
	"fD*", " rawname", "过滤原始名称以成为有效标志并以 r2 命令输出",
	NULL
};

static RCoreHelpMessage help_msg_fd = {
	"用法: fd[d]", " [offset|flag|expression]", " # 描述标志",
	"fd", " $$", "描述给定偏移量的标志 + 增量",
	"fd.", " $$", "检查当前地址中的标志（无增量）",
	"fdj", " $$", "以 json 描述当前标志",
	"fdd", " $$", "描述没有空间限制的标志",
	"fdw", " [string]", "通过字符串过滤当前偏移量的最近标志",
	NULL
};

static RCoreHelpMessage help_msg_fs = {
	"用法: fs", "[*] [+-][flagspace|addr]", " # 管理标志空间",
	"fs", "", "显示标志空间",
	"fs*", "", "以 r2 命令显示标志空间",
	"fsj", "", "以 JSON 显示标志空间",
	"fs", " *", "选择所有标志空间",
	"fs", " flagspace", "选择标志空间或如果不存在则创建",
	"fs", "-flagspace", "移除标志空间",
	"fs", "-*", "移除所有标志空间",
	"fs", "+foo", "推送上一个标志空间并设置",
	"fs", "-", "弹出到上一个标志空间",
	...
	"fsr", "[old] [new]", "重命名标志空间",
	"fsq", "", "以安静模式列出标志空间",
	"fsm", " [addr]", "将给定地址的标志移动到当前标志空间",
	"fsp", " <space> <prefix>", "将标志名称前缀与给定标志空间关联",
	"fss", "", "显示标志空间堆栈",
	"fss*", "", "以 r2 命令显示标志空间堆栈",
	"fssj", "", "以 JSON 显示标志空间堆栈",
	"fsr", " newname", "重命名选定的标志空间",
	NULL
};

static RCoreHelpMessage help_msg_fz = {
	"用法: f", "[?|-name| name] [@addr]", " # 管理标志区域",
	"fz", " math", "显示指定名称的标志区域信息",
	"fz+", "math", "添加新标志区域",
	"fz-", "math", "删除标志区域",
	"fz-", "*", "删除所有标志区域",
	"fz.", "", "显示当前标志区域上下文",
