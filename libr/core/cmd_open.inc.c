...
static RCoreHelpMessage help_msg_o = {
	"用法: o", "[file] ([offset])", "打开和关闭文件、映射和内存库",
	"o", "", "列出已打开的文件",
	"o", " [file] 0x4000 rwx", "在 0x4000 处映射文件",
	"o", " [file]", "以只读方式打开 [file] 文件",
	"o", "-1", "关闭文件描述符 1",
	"o*", "[*]", "以 r2 命令列出已打开的文件，显示为每个 fd 设置标志的 r2 脚本",
	"o+", " [file]", "以读写模式打开文件",
	"o++", " [file]", "创建并以读写模式打开文件 (参见 ot 和 omr)",
	"o-", "[?][#!*$.]", "关闭已打开的文件",
	"o.", "", "显示当前文件名 (或 o.q/oq 获取 fd)",
	"o:", " [len]", "打开 malloc://[len] 并从当前偏移复制字节", // XXX R2_590 - 应该是 ':' 的别名，不需要 malloc:// 包装器
	"o=", "(#fd)", "选择 fd 或以 ASCII 艺术形式列出已打开的文件",
	"oL", "", "列出所有已注册的 IO 插件",
	"oa", "[-] [A] [B] [filename]", "为给定文件指定架构和位数",
	"ob", "[?] [lbdos] [...]", "列出由 fd 支持的已打开二进制文件",
	"oc", " [file]", "打开核心文件，类似重新启动 r2",
	"of", "[?] [file]", "打开文件而不创建任何映射",
	"oe", " [filename]", "使用给定文件打开 cfg.editor",
	"oj", "", "以 JSON 格式列出已打开的文件",
	"om", "[?]", "创建、列出、删除 IO 映射",
	"on", "[?][n] [file] 0x4000", "在 0x4000 处映射原始文件 (不涉及 r_bin)",
	"oo", "[?][+bcdnm]", "重新打开当前文件 (参见 oo?) (以读写或调试器模式重新加载)",
	"op", "[npr] [fd]", "按 fd 选择优先文件 (参见 ob)，opn/opp/opr = 下一个/上一个/轮换",
	"ot", " [file]", "与 `touch [file]` 相同",
	"oq", "[q]", "列出所有打开的文件或显示当前 fd 'oqq'",
	"ox", " fd fdx", "交换 fd 和 fdx 的描述符并保留映射",
	"open", " [file]", "在文件上使用系统 xdg-open/open",
	NULL
};

static RCoreHelpMessage help_msg_on = {
	"用法: on[n+*]", "[file] ([addr] [rwx])", "打开文件而不解析头部",
	"on", " /bin/ls [addr] [perm] [vsize]", "在 addr 处映射原始文件，使用 vsize (不涉及 r_bin)",
	"onn", " [file] ([rwx])", "打开文件而不创建任何映射或使用 rbin 解析头部)",
	"onnu", " [file] ([rwx])", "与 onn 相同，但唯一，如果已打开则返回之前的 fd",
	"on+", " [file] ([rwx])", "以读写模式打开文件而不解析头部",
	"on*", "", "以 r2 命令列出打开的文件",
	NULL
};
...
static RCoreHelpMessage help_msg_omn = {
	"用法: omn[.i]", "([addr]) [name]", "为给定映射定义名称",
	"omn", " mapaddr [name]", "设置/删除跨越 mapaddr 的映射的名称",
	"omn.", "([-|name])", "显示/设置/删除当前映射的名称",
	"omni", " mapid [name]", "设置/删除具有 mapid 的映射的名称",
	NULL
};

static RCoreHelpMessage help_msg_omb = {
	"用法: omb[+-adgq]", "[fd]", "操作内存库",
	"omb", "", "列出所有内存库",
	"omb", " [id]", "切换到使用不同的内存库",
	"omb=", "[name]", "与 'omb id' 相同，但使用其名称",
	"omb-", "*", "删除所有内存库",
	"omb-", "[mapid]", "删除给定 id 的内存库",
	"omb+", "[name]", "创建具有给定名称的新内存库",
	"omba", " [id]", "向内存库添加映射",
	"ombd", " [id]", "从内存库删除映射",
	"ombg", "", "将所有映射关联到当前内存库",
	"ombj", "", "以 json 格式列出内存库",
...
static RCoreHelpMessage help_msg_oba = {
	"用法: oba", "[addr] ([filename])", "加载二进制信息并更新标志",
	"oba", " [addr]", "从给定地址打开二进制信息",
	"oba", " [addr] [baddr]", "打开文件并在给定地址加载二进制信息",
	"oba", " [addr] [/abs/filename]", "打开文件并在给定地址加载二进制信息",
	NULL
};

static RCoreHelpMessage help_msg_ob = {
	"用法: ob", "", "列出由 fd 支持的已打开二进制文件",
	"ob", " [name|bfid]", "切换到打开给定的 objid (或名称)",
	"ob", "", "列出已打开的二进制文件和 objid",
	"ob*", "", "列出已打开的二进制文件和 objid (r2 命令)",
	"ob", " *", "选择所有二进制文件 (使用 'ob bfid' 选择一个)",
	"obi", "?[..]", "'i' 的别名",
	"obio", "", "从 io 插件加载二进制信息，强制使用 bin.io",
	"obm", "([id])", "将当前选定的二进制文件合并到先前的二进制文件 (id-1) 中",
	"obm-", "([id])", "与 obm 相同，但删除当前二进制文件",
	"ob-", "*", "删除所有二进制文件",
	"ob-", "[objid]", "按 binobjid 删除二进制文件",
	"ob--", "", "删除最后一个二进制文件",
	"ob.", " ([addr])", "显示当前地址处的 bfid",
	"ob=", "", "以 ASCII 艺术表格显示打开文件的列表",
	"obL", "", "与 iL 或 Li 相同",
	"oba", " [addr] [baddr]", "打开文件并在给定地址加载二进制信息",
	"oba", " [addr] [filename]", "打开文件并在给定地址加载二进制信息",
	"oba", " [addr]", "从给定地址打开二进制信息",
	"obf", " ([file])", "为当前文件加载二进制信息 (对于 r2 -n 有用)",
	"obj", "", "以 JSON 格式列出已打开的二进制文件和 objid",
	"obo", " [fd]", "按 fd 编号切换到打开的二进制文件",
	"obr", " [baddr]", "重新定位当前二进制对象",
	NULL
};

static RCoreHelpMessage help_msg_omr = {
	"用法: om", "[arg]", "映射已打开的文件",
	"omr", " mapid", "优先处理具有相应 id 的映射",
	"omrb", " [fd]", "优先处理与 binid 关联的二进制文件的映射",
	"omrl", " fd", "将具有给定 fd 的映射重新排序到较低位置",
	"omrd", " [mapid]", "将具有相应 id 的映射向下重新排序",
	"omrf", " [fd]", "按 fd 重新排序映射",
	NULL
};

static RCoreHelpMessage help_msg_omt = {
	"用法: omt", "[arg]", "操作 IO 映射类型",
	"omt", " mapid", "切换映射向后绑定 (与 omtb 相同)",
	"omtb", " mapid", "切换映射向后绑定",
	"omtf", " mapid", "切换映射向前绑定",
	NULL
};
...
static RCoreHelpMessage help_msg_om = {
	"用法: om", "[arg]", "映射已打开的文件",
	"om", " [fd]", "列出特定 fd 的所有已定义 IO 映射",
	"om", " fd va [sz] [pa] [rwx] [name]", "创建新的 io 映射",
	"om", "[,=*jqq]", "列出 IO 映射",
	"om.", "", "显示映射到当前偏移的映射",
	"om-", "[mapid]", "移除具有相应 id 的映射",
	"om-*", "", "移除所有映射",
	"om+", " [fd]", "创建覆盖给定 fd 所有 VA 的映射",
	"oma", "[.] ([mapid]) (attr)", "为给定的映射 id 设置属性",
	"omb", "[?]", "列出/选择内存映射库",
	"omd", " from to @ paddr", "简化的 om；获取当前搜索位置、fd 和权限",
	"omm", " [fd]", "为给定 fd 创建默认映射 (omm `oq`)",
	"omn", "[?] ([fd]) [name]", "管理映射名称",
	"omo", "[j*]", "差异覆盖映射数据 (通常是重定位)",
	"omp", " [mapid] rwx", "更改映射 rwx 权限 (参见 dmp 用于调试)",
	"ompg", "[+-]rwx", "全局更改所有映射的权限",
	"omr", "[?]", "重新排序映射优先级",
	"oms", " [mapid] [newsize]", "显示或更改具有相应 id 的映射大小",
	"omt", "[?]", "切换映射向后或向前绑定",
	"omu", " fd va sz pa rwx name", "与 `om` 相同，但检查是否存在 (u 代表唯一)",
	"omv", "[?]", "将映射移动到给定地址",
	// "om*", "", "以 r2 命令格式列出所有映射",
	// "om,", " [query]", "使用表格 API 列出映射",
	// "om=", "", "以 ASCII 艺术形式列出所有映射",
	// "omj", "", "以 json 格式列出所有映射",
	// "omq", "", "列出所有映射及其 fd",
	// "omqq", "", "列出所有映射地址 (参见 $MM 获取大小)",
	NULL
};

static RCoreHelpMessage help_msg_omv = {
	"用法: omv", " ([mapid]) [addr]", "将映射移动到另一个地址",
	"omv", " mapid addr", "重新定位具有相应 id 的映射",
	"omv.", " addr", "重新定位当前映射",
	NULL
};
...
static RCoreHelpMessage help_msg_oo = {
	...
	"oo", "", "重新打开当前文件",
	"oo+", "", "以读写模式重新打开",
	"oob", " [baddr]", "重新打开并加载 rbin 信息 (更改基地址?)",
	"ooi", "", "重新打开二进制信息而不重新加载文件",
	"ooc", "", "使用当前文件重新打开核心",
	"ood", "[?]", "以调试模式重新打开",
	"oom", "[?]", "在 malloc:// 中重新打开",
	"oon", "", "重新打开而不加载 rbin 信息",
	"oon+", "", "以读写模式重新打开而不加载 rbin 信息",
	"oonn", "", "重新打开而不加载 rbin 信息，但带有头部标志",
	"oonn+", "", "以读写模式重新打开而不加载 rbin 信息，但带有",
	NULL
};

static RCoreHelpMessage help_msg_ood = {
	"用法: ood", "", "调试（重新）打开命令",
	"ood", " [args]", "以调试模式重新打开 (带参数)",
	"oodf", " [file]", "使用给定文件以调试模式重新打开",
	"oodr", " [rarun2]", "与 dor ..;ood 相同",
	NULL
};
